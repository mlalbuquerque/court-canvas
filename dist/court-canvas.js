...
var c = Object.create, l = Object.defineProperty, u = Object.getOwnPropertyDescriptor, d = Object.getOwnPropertyNames, f = Object.getPrototypeOf, p = Object.prototype.hasOwnProperty, m = (e, t) => () => (t || e((t = { exports: {} }).exports, t), t.exports), h = (e, t, n, r) => {
	...
	e.glob = typeof global < "u" ? global : typeof window < "u" ? window : typeof WorkerGlobalScope < "u" ? self : {}, e.Konva = {
		_global: e.glob,
		version: "9.3.22",
		isBrowser: n(),
		...
		isTransforming() {
			return e.Konva.Transformer?.isTransforming();
		},
		isDragReady() {
	...
	e.Transform = class t {
		...
		decompose() {
			let t = this.m[0], n = this.m[1], r = this.m[2], i = this.m[3], a = this.m[4], o = this.m[5], s = t * i - n * r, c = {
				x: a,
				y: o,
				rotation: 0,
				scaleX: 0,
			...
			if (t != 0 || n != 0) {
				let e = Math.sqrt(t * t + n * n);
				c.rotation = n > 0 ? Math.acos(t / e) : -Math.acos(t / e), c.scaleX = e, c.scaleY = s / e, c.skewX = (t * r + n * i) / s, c.skewY = 0;
			} else if (r != 0 || i != 0) {
				let e = Math.sqrt(r * r + i * i);
				c.rotation = Math.PI / 2 - (i > 0 ? Math.acos(-r / e) : -Math.acos(r / e)), c.scaleX = s / e, c.scaleY = e, c.skewX = 0, c.skewY = (t * r + n * i) / s;
			}
			return c.rotation = e.Util._getRotation(c.rotation), c;
		}
	};
	var n = "[object Array]", r = "[object Number]", i = "[object String]", a = "[object Boolean]", o = Math.PI / 180, s = 180 / Math.PI, c = "#", l = "", u = "0", d = "Konva warning: ", f = "Konva error: ", p = "rgb(", m = {
		aliceblue: [
			240,
			248,
			255
	...
	}, h = /rgb\((\d{1,3}),(\d{1,3}),(\d{1,3})\)/, g = [], _ = typeof requestAnimationFrame < "u" && requestAnimationFrame || function(e) {
		setTimeout(e, 60);
	};
	e.Util = {
		_isElement(e) {
		...
		_isNumber(e) {
			return Object.prototype.toString.call(e) === r && !isNaN(e) && isFinite(e);
		},
		_isString(e) {
		...
		requestAnimFrame(e) {
			g.push(e), g.length === 1 && _(function() {
				let e = g;
		...
		_urlToImage(t, n) {
			let r = e.Util.createImageElement();
			r.onload = function() {
				n(r);
			}, r.src = t;
		},
		...
		getRGB(e) {
			...
			return e in m ? (t = m[e], {
				...
				g: t[1],
				b: t[2]
			}) : e[0] === c ? this._hexToRgb(e.substring(1)) : e.substr(0, 4) === p ? (t = h.exec(e.replace(/ /g, "")), {
				r: parseInt(t[1], 10),
				g: parseInt(t[2], 10),
				b: parseInt(t[3], 10)
			}) : {
		...
		colorToRGBA(t) {
			return t ||= "black", e.Util._namedColorToRBA(t) || e.Util._hex3ColorToRGBA(t) || e.Util._hex4ColorToRGBA(t) || e.Util._hex6ColorToRGBA(t) || e.Util._hex8ColorToRGBA(t) || e.Util._rgbColorToRGBA(t) || e.Util._rgbaColorToRGBA(t) || e.Util._hslColorToRGBA(t);
		},
		_namedColorToRBA(e) {
		...
		_degToRad(t) {
			return e.Util.warn("Util._degToRad is removed. Please use public Util.degToRad instead."), e.Util.degToRad(t);
		},
		_radToDeg(t) {
			return e.Util.warn("Util._radToDeg is removed. Please use public Util.radToDeg instead."), e.Util.radToDeg(t);
		},
		_getRotation(n) {
			return t.Konva.angleDeg ? e.Util.radToDeg(n) : n;
		},
		_capitalize(e) {
		...
		warn(e) {
			t.Konva.showWarnings && console.warn(d + e);
		},
		each(e, t) {
		...
		_getProjectionToLine(t, n, r) {
			let i = e.Util.cloneObject(t), a = Number.MAX_VALUE;
			return n.forEach(function(o, s) {
				if (!r && s === n.length - 1) return;
				let c = n[(s + 1) % n.length], l = e.Util._getProjectionToSegment(o.x, o.y, c.x, c.y, t.x, t.y), u = l[0], d = l[1], f = l[2];
				f < a && (i.x = u, i.y = d, a = f);
			}), i;
		},
		_prepareArrayForTween(t, n, r) {
			let i = [], a = [];
			...
			for (let e = 0; e < n.length; e += 2) a.push({
				x: n[e],
				y: n[e + 1]
			});
			let o = [];
			return a.forEach(function(t) {
				let n = e.Util._getProjectionToLine(t, i, r);
				o.push(n.x), o.push(n.y);
			}), o;
		},
		_prepareToStringify(t) {
			let n;
			t.visitedByCircularReferenceRemoval = !0;
			for (let r in t) if (t.hasOwnProperty(r) && t[r] && typeof t[r] == "object") {
				if (n = Object.getOwnPropertyDescriptor(t, r), t[r].visitedByCircularReferenceRemoval || e.Util._isElement(t[r])) if (n.configurable) delete t[r];
				else return null;
				else if (e.Util._prepareToStringify(t[r]) === null) if (n.configurable) delete t[r];
				else return null;
			}
		...
		releaseCanvas(...e) {
			t.Konva.releaseCanvasOnDestroy && e.forEach((e) => {
				e.width = 0, e.height = 0;
			});
		},
		drawRoundedRectPath(e, t, n, r) {
			let i = 0, a = 0, o = 0, s = 0;
			typeof r == "number" ? i = a = o = s = Math.min(r, t / 2, n / 2) : (i = Math.min(r[0] || 0, t / 2, n / 2), a = Math.min(r[1] || 0, t / 2, n / 2), s = Math.min(r[2] || 0, t / 2, n / 2), o = Math.min(r[3] || 0, t / 2, n / 2)), e.moveTo(i, 0), e.lineTo(t - a, 0), e.arc(t - a, a, a, Math.PI * 3 / 2, 0, !1), e.lineTo(t, n - s), e.arc(t - s, n - s, s, 0, Math.PI / 2, !1), e.lineTo(o, n), e.arc(o, n - o, o, Math.PI / 2, Math.PI, !1), e.lineTo(0, i), e.arc(i, i, i, Math.PI, Math.PI * 3 / 2, !1);
		}
	};
})), b = /* @__PURE__ */ m(((e) => {
	...
	var t = y(), n = v();
	function r(e) {
		let n = [], r = e.length, i = t.Util;
		for (let t = 0; t < r; t++) {
			let r = e[t];
			i._isNumber(r) ? r = Math.round(r * 1e3) / 1e3 : i._isString(r) || (r += ""), n.push(r);
		}
		return n;
	}
	var i = ",", a = "(", o = ")", s = "([", c = "])", l = ";", u = "()", d = "=", f = /* @__PURE__ */ "arc.arcTo.beginPath.bezierCurveTo.clearRect.clip.closePath.createLinearGradient.createPattern.createRadialGradient.drawImage.ellipse.fill.fillText.getImageData.createImageData.lineTo.moveTo.putImageData.quadraticCurveTo.rect.roundRect.restore.rotate.save.scale.setLineDash.setTransform.stroke.strokeText.transform.translate".split("."), p = [
		"fillStyle",
		"strokeStyle",
		"shadowColor",
		"shadowBlur",
		...
		constructor(e) {
			this.canvas = e, n.Konva.enableTrace && (this.traceArr = [], this._enableTrace());
		}
		fillShape(e) {
		...
		getTrace(e, n) {
			let r = this.traceArr, f = r.length, p = "", m, h, g, _;
			for (m = 0; m < f; m++) h = r[m], g = h.method, g ? (_ = h.args, p += g, e ? p += u : t.Util._isArray(_[0]) ? p += s + _.join(i) + c : (n && (_ = _.map((e) => typeof e == "number" ? Math.floor(e) : e)), p += a + _.join(i) + o)) : (p += h.property, e || (p += d + h.val)), p += l;
			return p;
		}
		clearTrace() {
			this.traceArr = [];
		...
		_trace(e) {
			...
			t.push(e), n = t.length, n >= m && t.shift();
		}
		reset() {
		...
		_enableTrace() {
			let e = this, t = f.length, n = this.setAttr, i, a, o = function(t) {
				let n = e[t], i;
				e[t] = function() {
					return a = r(Array.prototype.slice.call(arguments, 0)), i = n.apply(e, arguments), e._trace({
						method: t,
						args: a
					}), i;
				};
	...
	e.Context = h, p.forEach(function(e) {
	...
	}), e.SceneContext = class extends h {
	...
	function a() {
		if (i) return i;
		let e = t.Util.createCanvasElement(), n = e.getContext("2d");
		return i = (function() {
			return (r.Konva._global.devicePixelRatio || 1) / (n.webkitBackingStorePixelRatio || n.mozBackingStorePixelRatio || n.msBackingStorePixelRatio || n.oBackingStorePixelRatio || n.backingStorePixelRatio || 1);
		})(), t.Util.releaseCanvas(e), i;
	}
	var o = class {
		constructor(e) {
			this.pixelRatio = 1, this.width = 0, this.height = 0, this.isCache = !1, this.pixelRatio = (e || {}).pixelRatio || r.Konva.pixelRatio || a(), this._canvas = t.Util.createCanvasElement(), this._canvas.style.padding = "0", this._canvas.style.margin = "0", this._canvas.style.border = "0", this._canvas.style.background = "transparent", this._canvas.style.position = "absolute", this._canvas.style.top = "0", this._canvas.style.left = "0";
		}
		getContext() {
			return this.context;
		}
		...
		toDataURL(e, n) {
			try {
				...
				try {
					...
					return t.Util.error("Unable to get data URL. " + e.message + " For more info read https://konvajs.org/docs/posts/Tainted_Canvas.html."), "";
				}
			}
		}
	};
	e.Canvas = o, e.SceneCanvas = class extends o {
		constructor(e = {
			width: 0,
			height: 0,
			willReadFrequently: !1
		}) {
			super(e), this.context = new n.SceneContext(this, { willReadFrequently: e.willReadFrequently }), this.setSize(e.width, e.height);
		}
	}, e.HitCanvas = class extends o {
		constructor(e = {
			width: 0,
			height: 0
		}) {
			super(e), this.hitCanvas = !0, this.context = new n.HitContext(this), this.setSize(e.width, e.height);
		}
	};
})), S = /* @__PURE__ */ m(((e) => {
	...
	e.DD = {
		...
		_drag(t) {
			let r = [];
			e.DD._dragElements.forEach((e, i) => {
				let { node: a } = e, o = a.getStage();
				o.setPointersPositions(t), e.pointerId === void 0 && (e.pointerId = n.Util._getFirstPointerId(t));
				let s = o._changedPointerPositions.find((t) => t.id === e.pointerId);
				if (s) {
					if (e.dragStatus !== "dragging") {
						let n = a.dragDistance();
						if (Math.max(Math.abs(s.x - e.startPointerPos.x), Math.abs(s.y - e.startPointerPos.y)) < n || (a.startDrag({ evt: t }), !a.isDragging())) return;
					}
		...
		_endDragBefore(n) {
			...
			e.DD._dragElements.forEach((i) => {
				let { node: a } = i, o = a.getStage();
				if (n && o.setPointersPositions(n), !o._changedPointerPositions.find((e) => e.id === i.pointerId)) return;
				(i.dragStatus === "dragging" || i.dragStatus === "stopped") && (e.DD.justDragged = !0, t.Konva._mouseListenClick = !1, t.Konva._touchListenClick = !1, t.Konva._pointerListenClick = !1, i.dragStatus = "stopped");
				let s = i.node.getLayer() || i.node instanceof t.Konva.Stage && i.node;
				s && r.indexOf(s) === -1 && r.push(s);
			}), r.forEach((e) => {
				e.draw();
			});
		},
		_endDragAfter(t) {
			e.DD._dragElements.forEach((n, r) => {
				n.dragStatus === "stopped" && n.node.fire("dragend", {
					type: "dragend",
					target: n.node,
					evt: t
				}, !0), n.dragStatus !== "dragging" && e.DD._dragElements.delete(r);
			});
		}
	...
	Object.defineProperty(e, "__esModule", { value: !0 }), e.RGBComponent = i, e.alphaComponent = a, e.getNumberValidator = o, e.getNumberOrArrayOfNumbersValidator = s, e.getNumberOrAutoValidator = c, e.getStringValidator = l, e.getStringOrGradientValidator = u, e.getFunctionValidator = d, e.getNumberArrayValidator = f, e.getBooleanValidator = p, e.getComponentValidator = m;
	var t = v(), n = y();
	function r(e) {
		return n.Util._isString(e) ? "\"" + e + "\"" : Object.prototype.toString.call(e) === "[object Number]" || n.Util._isBoolean(e) ? e : Object.prototype.toString.call(e);
	}
	function i(e) {
	...
	function o() {
		if (t.Konva.isUnminified) return function(e, t) {
			return n.Util._isNumber(e) || n.Util.warn(r(e) + " is a not valid value for \"" + t + "\" attribute. The value should be a number."), e;
		};
	}
	function s(e) {
		if (t.Konva.isUnminified) return function(t, i) {
			let a = n.Util._isNumber(t), o = n.Util._isArray(t) && t.length == e;
			return !a && !o && n.Util.warn(r(t) + " is a not valid value for \"" + i + "\" attribute. The value should be a number or Array<number>(" + e + ")"), t;
		};
	}
	function c() {
		if (t.Konva.isUnminified) return function(e, t) {
			return n.Util._isNumber(e) || e === "auto" || n.Util.warn(r(e) + " is a not valid value for \"" + t + "\" attribute. The value should be a number or \"auto\"."), e;
		};
	}
	function l() {
		if (t.Konva.isUnminified) return function(e, t) {
			return n.Util._isString(e) || n.Util.warn(r(e) + " is a not valid value for \"" + t + "\" attribute. The value should be a string."), e;
		};
	}
	function u() {
		if (t.Konva.isUnminified) return function(e, t) {
			let i = n.Util._isString(e), a = Object.prototype.toString.call(e) === "[object CanvasGradient]" || e && e.addColorStop;
			return i || a || n.Util.warn(r(e) + " is a not valid value for \"" + t + "\" attribute. The value should be a string or a native gradient."), e;
		};
	}
	function d() {
		if (t.Konva.isUnminified) return function(e, t) {
			return n.Util._isFunction(e) || n.Util.warn(r(e) + " is a not valid value for \"" + t + "\" attribute. The value should be a function."), e;
		};
	}
	function f() {
		if (t.Konva.isUnminified) return function(e, t) {
			let i = Int8Array ? Object.getPrototypeOf(Int8Array) : null;
			return i && e instanceof i || (n.Util._isArray(e) ? e.forEach(function(e) {
				n.Util._isNumber(e) || n.Util.warn("\"" + t + "\" attribute has non numeric element " + e + ". Make sure that all elements are numbers.");
			}) : n.Util.warn(r(e) + " is a not valid value for \"" + t + "\" attribute. The value should be a array of numbers.")), e;
		};
	}
	function p() {
		if (t.Konva.isUnminified) return function(e, t) {
			return e === !0 || e === !1 || n.Util.warn(r(e) + " is a not valid value for \"" + t + "\" attribute. The value should be a boolean."), e;
		};
	}
	function m(e) {
		if (t.Konva.isUnminified) return function(t, i) {
			return t == null || n.Util.isObject(t) || n.Util.warn(r(t) + " is a not valid value for \"" + i + "\" attribute. The value should be an object with properties " + e), t;
		};
	}
})), w = /* @__PURE__ */ m(((e) => {
	Object.defineProperty(e, "__esModule", { value: !0 }), e.Factory = void 0;
	var t = y(), n = C(), r = "get", i = "set";
	e.Factory = {
		addGetterSetter(t, n, r, i, a) {
			e.Factory.addGetter(t, n, r), e.Factory.addSetter(t, n, i, a), e.Factory.addOverloadedGetterSetter(t, n);
		},
		addGetter(e, n, i) {
			let a = r + t.Util._capitalize(n);
			e.prototype[a] = e.prototype[a] || function() {
				let e = this.attrs[n];
				return e === void 0 ? i : e;
			};
		},
		addSetter(n, r, a, o) {
			let s = i + t.Util._capitalize(r);
			n.prototype[s] || e.Factory.overWriteSetter(n, r, a, o);
		},
		overWriteSetter(e, n, r, a) {
			let o = i + t.Util._capitalize(n);
			e.prototype[o] = function(e) {
				return r && e != null && (e = r.call(this, e, n)), this._setAttr(n, e), a && a.call(this), this;
			};
		},
		addComponentsGetterSetter(a, o, s, c, l) {
			let u = s.length, d = t.Util._capitalize, f = r + d(o), p = i + d(o);
			a.prototype[f] = function() {
				let e = {};
				for (let t = 0; t < u; t++) {
					let n = s[t];
					e[n] = this.getAttr(o + d(n));
				}
				return e;
			};
			let m = (0, n.getComponentValidator)(s);
			a.prototype[p] = function(e) {
				let t = this.attrs[o];
				c && (e = c.call(this, e, o)), m && m.call(this, e, o);
				for (let t in e) e.hasOwnProperty(t) && this._setAttr(o + d(t), e[t]);
				return e || s.forEach((e) => {
					this._setAttr(o + d(e), void 0);
				}), this._fireChangeEvent(o, t, e), l && l.call(this), this;
			}, e.Factory.addOverloadedGetterSetter(a, o);
		},
		addOverloadedGetterSetter(e, n) {
			let a = t.Util._capitalize(n), o = i + a, s = r + a;
			e.prototype[n] = function() {
				return arguments.length ? (this[o](arguments[0]), this) : this[s]();
			};
		},
		addDeprecatedGetterSetter(n, i, a, o) {
			t.Util.error("Adding deprecated " + i);
			let s = r + t.Util._capitalize(i), c = i + " property is deprecated and will be removed soon. Look at Konva change log for more information.";
			n.prototype[s] = function() {
				t.Util.error(c);
				let e = this.attrs[i];
				return e === void 0 ? a : e;
			}, e.Factory.addSetter(n, i, o, function() {
				t.Util.error(c);
			}), e.Factory.addOverloadedGetterSetter(n, i);
		},
		backCompat(e, n) {
			t.Util.each(n, function(n, a) {
				let o = e.prototype[a], s = r + t.Util._capitalize(n), c = i + t.Util._capitalize(n);
				function l() {
					o.apply(this, arguments), t.Util.error("\"" + n + "\" method is deprecated and will be removed soon. Use \"\"" + a + "\" instead.");
				}
				e.prototype[n] = l, e.prototype[s] = l, e.prototype[c] = l;
			});
		},
		afterSetFilter() {
	...
	var t = x(), n = S(), r = w(), i = v(), a = y(), o = C(), s = "absoluteOpacity", c = "allEventListeners", l = "absoluteTransform", u = "absoluteScale", d = "canvas", f = "Change", p = "children", m = "konva", h = "listening", g = "mouseenter", _ = "mouseleave", b = "pointerenter", T = "pointerleave", E = "touchenter", D = "touchleave", O = "set", k = "Shape", A = " ", j = "stage", M = "transform", N = "Stage", P = "visible", F = [
		"xChange.konva",
		"yChange.konva",
		"scaleXChange.konva",
		"scaleYChange.konva",
		...
		_clearCache(e) {
			(e === M || e === l) && this._cache.get(e) ? this._cache.get(e).dirty = !0 : e ? this._cache.delete(e) : this._cache.clear();
		}
		_getCache(e, t) {
			let n = this._cache.get(e);
			return (n === void 0 || (e === M || e === l) && n.dirty === !0) && (n = t.call(this), this._cache.set(e, n)), n;
		}
		_calculate(e, t, n) {
			if (!this._attachedDepsListeners.get(e)) {
				let n = t.map((e) => e + "Change.konva").join(A);
				this.on(n, () => {
					this._clearCache(e);
		...
		_clearSelfAndDescendantCache(e) {
			this._clearCache(e), e === l && this.fire("absoluteTransformChange");
		}
		clearCache() {
			if (this._cache.has(d)) {
				let { scene: e, filter: t, hit: n, buffer: r } = this._cache.get(d);
				a.Util.releaseCanvas(e, t, n, r), this._cache.delete(d);
			}
		...
		cache(e) {
			...
			let i = Math.ceil(n.width || r.width), o = Math.ceil(n.height || r.height), c = n.pixelRatio, l = n.x === void 0 ? Math.floor(r.x) : n.x, f = n.y === void 0 ? Math.floor(r.y) : n.y, p = n.offset || 0, m = n.drawBorder || !1, h = n.hitCanvasPixelRatio || 1;
			if (!i || !o) {
				a.Util.error("Can not cache the node. Width or height of the node equals 0. Caching is skipped.");
				return;
			}
			let g = Math.abs(Math.round(r.x) - l) > .5 ? 1 : 0, _ = Math.abs(Math.round(r.y) - f) > .5 ? 1 : 0;
			i += p * 2 + g, o += p * 2 + _, l -= p, f -= p;
			let v = new t.SceneCanvas({
				pixelRatio: c,
				width: i,
				height: o
			}), y = new t.SceneCanvas({
		...
		_getCachedSceneCanvas() {
			let e = this.filters(), t = this._getCanvasCache(), n = t.scene, r = t.filter, i = r.getContext(), o, s, c, l;
			if (e) {
				if (!this._filterUpToDate) {
					let t = n.pixelRatio;
					r.setSize(n.width / n.pixelRatio, n.height / n.pixelRatio);
					try {
						for (o = e.length, i.clear(), i.drawImage(n._canvas, 0, 0, n.getWidth() / t, n.getHeight() / t), s = i.getImageData(0, 0, r.getWidth(), r.getHeight()), c = 0; c < o; c++) {
							if (l = e[c], typeof l != "function") {
								a.Util.error("Filter should be type of function, but got " + typeof l + " instead. Please check correct filters");
								continue;
							}
							l.call(this, s), i.putImageData(s, 0, 0);
						}
					} catch (e) {
						a.Util.error("Unable to apply filter. " + e.message + " This post my help you https://konvajs.org/docs/posts/Tainted_Canvas.html.");
					}
		...
		on(e, t) {
			if (this._cache && this._cache.delete(c), arguments.length === 3) return this._delegate.apply(this, arguments);
			let n = e.split(A);
			for (let e = 0; e < n.length; e++) {
				let r = n[e].split("."), i = r[0], a = r[1] || "";
		...
		off(e, t) {
			let n = (e || "").split(A), r = n.length, i, a, o, s, l, u;
			if (this._cache && this._cache.delete(c), !e) for (a in this.eventListeners) this._off(a);
			for (i = 0; i < r; i++) if (o = n[i], s = o.split("."), l = s[0], u = s[1], l) this.eventListeners[l] && this._off(l, u, t);
		...
		_delegate(e, t, n) {
			...
			this.on(e, function(e) {
				...
				for (let t = 0; t < i.length; t++) e = a.Util.cloneObject(e), e.currentTarget = i[t], n.call(i[t], e);
			});
		}
		remove() {
			return this.isDragging() && this.stopDrag(), n.DD._dragElements.delete(this._id), this._remove(), this;
		}
		_clearCaches() {
			this._clearSelfAndDescendantCache(l), this._clearSelfAndDescendantCache(s), this._clearSelfAndDescendantCache(u), this._clearSelfAndDescendantCache(j), this._clearSelfAndDescendantCache(P), this._clearSelfAndDescendantCache(h);
		}
		_remove() {
			this._clearCaches();
		...
		getAttr(e) {
			let t = "get" + a.Util._capitalize(e);
			return a.Util._isFunction(this[t]) ? this[t]() : this.attrs[e];
		}
		getAncestors() {
		...
		setAttrs(e) {
			return this._batchTransformChanges(() => {
				let t, n;
				if (!e) return this;
				for (t in e) t !== p && (n = O + a.Util._capitalize(t), a.Util._isFunction(this[n]) ? this[n](e[t]) : this._setAttr(t, e[t]));
			}), this;
		}
		isListening() {
		...
		shouldDrawHit(e, t = !1) {
			...
			let r = this.getLayer(), a = !1;
			n.DD._dragElements.forEach((e) => {
				e.dragStatus === "dragging" && (e.node.nodeType === "Stage" || e.node.getLayer() === r) && (a = !0);
			});
			let o = !t && !i.Konva.hitOnDragEnabled && (a || i.Konva.isTransforming());
		...
		getAbsoluteZIndex() {
			...
			function s(c) {
				for (r = [], i = c.length, a = 0; a < i; a++) o = c[a], n++, o.nodeType !== k && (r = r.concat(o.getChildren().slice())), o._id === t._id && (a = i);
				r.length > 0 && r[0].getDepth() <= e && s(r);
			}
			let c = this.getStage();
			return t.nodeType !== N && c && s(c.getChildren()), n;
		}
		getDepth() {
		...
		_batchTransformChanges(e) {
			this._batchingTransformChange = !0, e(), this._batchingTransformChange = !1, this._needClearTransformCache && (this._clearCache(M), this._clearSelfAndDescendantCache(l)), this._needClearTransformCache = !1;
		}
		setPosition(e) {
			return this._batchTransformChanges(() => {
		...
		getAbsolutePosition(e) {
			...
			t && !e && (e = !0);
			let r = this.getAbsoluteTransform(e).getMatrix(), i = new a.Transform(), o = this.offset();
			return i.m = r.slice(), i.translate(o.x, o.y), i.getTranslation();
		}
		setAbsolutePosition(e) {
			let { x: t, y: n, ...r } = this._clearTransform();
			this.attrs.x = t, this.attrs.y = n, this._clearCache(M);
			...
			return i.invert(), i.translate(e.x, e.y), e = {
				...
				x: e.x,
				y: e.y
			}), this._clearCache(M), this._clearSelfAndDescendantCache(l), this;
		}
		_setTransform(e) {
			let t;
		...
		setZIndex(e) {
			if (!this.parent) return a.Util.warn("Node has no parent. zIndex parameter is ignored."), this;
			(e < 0 || e >= this.parent.children.length) && a.Util.warn("Unexpected value " + e + " for zIndex property. zIndex is just index of a node in children of its parent. Expected value is from 0 to " + (this.parent.children.length - 1) + ".");
			let t = this.index;
		...
		toObject() {
			let e = this.getAttrs(), t, n, r, i, o, s = {
				attrs: {},
				className: this.getClassName()
			};
			for (t in e) n = e[t], o = a.Util.isObject(n) && !a.Util._isPlainObject(n) && !a.Util._isArray(n), !o && (r = typeof this[t] == "function" && this[t], delete e[t], i = r ? r.call(this) : null, e[t] = n, i !== n && (s.attrs[t] = n));
			return a.Util._prepareToStringify(s);
		}
		toJSON() {
		...
		_isMatch(e) {
			...
			for (r = 0; r < n; r++) if (i = t[r], a.Util.isValidSelector(i) || (a.Util.warn("Selector \"" + i + "\" is invalid. Allowed selectors examples are \"#foo\", \".bar\" or \"Group\"."), a.Util.warn("If you have a custom shape with such className, please change it to start with upper letter like \"Triangle\"."), a.Util.warn("Konva is awesome, right?")), i.charAt(0) === "#") {
		...
		getStage() {
			return this._getCache(j, this._getStage);
		}
		_getStage() {
			let e = this.getParent();
		...
		getAbsoluteTransform(e) {
			return e ? this._getAbsoluteTransform(e) : this._getCache(l, this._getAbsoluteTransform);
		}
		_getAbsoluteTransform(e) {
			let t;
			if (e) return t = new a.Transform(), this._eachAncestorReverse(function(e) {
				let n = e.transformsEnabled();
				n === "all" ? t.multiply(e.getTransform()) : n === "position" && t.translate(e.x() - e.offsetX(), e.y() - e.offsetY());
			}, e), t;
			{
				t = this._cache.get(l) || new a.Transform(), this.parent ? this.parent.getAbsoluteTransform().copyInto(t) : t.reset();
		...
		_getTransform() {
			let e = this._cache.get(M) || new a.Transform();
			e.reset();
			let t = this.x(), n = this.y(), r = i.Konva.getAngle(this.rotation()), o = this.attrs.scaleX ?? 1, s = this.attrs.scaleY ?? 1, c = this.attrs.skewX || 0, l = this.attrs.skewY || 0, u = this.attrs.offsetX || 0, d = this.attrs.offsetY || 0;
			return (t !== 0 || n !== 0) && e.translate(t, n), r !== 0 && e.rotate(r), (c !== 0 || l !== 0) && e.skew(c, l), (o !== 1 || s !== 1) && e.scale(o, s), (u !== 0 || d !== 0) && e.translate(-1 * u, -1 * d), e.dirty = !1, e;
		}
		clone(e) {
			let t = a.Util.cloneObject(this.attrs), n, r, i, o, s;
			for (n in e) t[n] = e[n];
			let c = new this.constructor(t);
			for (n in this.eventListeners) for (r = this.eventListeners[n], i = r.length, o = 0; o < i; o++) s = r[o], s.name.indexOf(m) < 0 && (c.eventListeners[n] || (c.eventListeners[n] = []), c.eventListeners[n].push(s));
			return c;
		}
		_toKonvaCanvas(e) {
			e ||= {};
			let n = this.getClientRect(), r = this.getStage(), i = e.x === void 0 ? Math.floor(n.x) : e.x, a = e.y === void 0 ? Math.floor(n.y) : e.y, o = e.pixelRatio || 1, s = new t.SceneCanvas({
				width: e.width || Math.ceil(n.width) || (r ? r.width() : 0),
				height: e.height || Math.ceil(n.height) || (r ? r.height() : 0),
				pixelRatio: o
			}), c = s.getContext(), l = new t.SceneCanvas({
				width: s.width / s.pixelRatio + Math.abs(i),
				height: s.height / s.pixelRatio + Math.abs(a),
				pixelRatio: s.pixelRatio
			});
			return e.imageSmoothingEnabled === !1 && (c._context.imageSmoothingEnabled = !1), c.save(), (i || a) && c.translate(-1 * i, -1 * a), this.drawScene(s, void 0, l), c.restore(), s;
		}
		toCanvas(e) {
		...
		toImage(e) {
			return new Promise((t, n) => {
				try {
					let n = e?.callback;
					n && delete e.callback, a.Util._urlToImage(this.toDataURL(e), function(e) {
						t(e), n?.(e);
					});
				} catch (e) {
					n(e);
		...
		getDragDistance() {
			return this.attrs.dragDistance === void 0 ? this.parent ? this.parent.getDragDistance() : i.Konva.dragDistance : this.attrs.dragDistance;
		}
		_off(e, t, n) {
		...
		setAttr(e, t) {
			let n = this[O + a.Util._capitalize(e)];
			return a.Util._isFunction(n) ? n.call(this, t) : this._setAttr(e, t), this;
		}
		_requestDraw() {
			i.Konva.autoDrawEnabled && (this.getLayer() || this.getStage())?.batchDraw();
		}
		_setAttr(e, t) {
			let n = this.attrs[e];
			n === t && !a.Util.isObject(t) || (t == null ? delete this.attrs[e] : this.attrs[e] = t, this._shouldFireChangeEvents && this._fireChangeEvent(e, n, t), this._requestDraw());
		}
		...
		_fireAndBubble(e, t, n) {
			t && this.nodeType === k && (t.target = this);
			let r = [
				g,
				_,
				b,
		...
		_getProtoListeners(e) {
			let t = this._cache.get(c) ?? {}, n = t?.[e];
			if (n === void 0) {
				n = [];
				let r = Object.getPrototypeOf(this);
				for (; r;) {
					let t = r.eventListeners?.[e] ?? [];
					n.push(...t), r = Object.getPrototypeOf(r);
				}
				t[e] = n, this._cache.set(c, t);
			}
			return n;
		}
		_fire(e, t) {
			...
			if (n) for (let e = 0; e < n.length; e++) n[e].handler.call(this, t);
			let r = this.eventListeners[e];
			if (r) for (let e = 0; e < r.length; e++) r[e].handler.call(this, t);
		}
		draw() {
			return this.drawScene(), this.drawHit(), this;
		}
		_createDragElement(e) {
			let t = e ? e.pointerId : void 0, r = this.getStage(), i = this.getAbsolutePosition();
			if (!r) return;
			let a = r._getPointerById(t) || r._changedPointerPositions[0] || i;
			n.DD._dragElements.set(this._id, {
				node: this,
				startPointerPos: a,
				offset: {
					x: a.x - i.x,
		...
		startDrag(e, t = !0) {
			n.DD._dragElements.has(this._id) || this._createDragElement(e);
			let r = n.DD._dragElements.get(this._id);
		...
		_setDragPosition(e, t) {
			...
			if (i !== void 0) {
				...
				t ? r = t : a.Util.warn("dragBoundFunc did not return any value. That is unexpected behavior. You must return new absolute position from dragBoundFunc.");
			}
			(!this._lastPos || this._lastPos.x !== r.x || this._lastPos.y !== r.y) && (this.setAbsolutePosition(r), this._requestDraw()), this._lastPos = r;
		}
		stopDrag(e) {
			let t = n.DD._dragElements.get(this._id);
			t && (t.dragStatus = "stopped"), n.DD._endDragBefore(e), n.DD._endDragAfter(e);
		}
		setDraggable(e) {
			this._setAttr("draggable", e), this._dragChange();
		}
		isDragging() {
			let e = n.DD._dragElements.get(this._id);
			return e ? e.dragStatus === "dragging" : !1;
		}
		_listenDrag() {
			this._dragCleanup(), this.on("mousedown.konva touchstart.konva", function(e) {
				if (!(e.evt.button === void 0 || i.Konva.dragButtons.indexOf(e.evt.button) >= 0) || this.isDragging()) return;
				let t = !1;
				n.DD._dragElements.forEach((e) => {
		...
		_dragChange() {
			if (this.attrs.draggable) this._listenDrag();
				...
				let e = n.DD._dragElements.get(this._id), t = e && e.dragStatus === "dragging", r = e && e.dragStatus === "ready";
				t ? this.stopDrag() : r && n.DD._dragElements.delete(this._id);
			}
		}
		_dragCleanup() {
		...
		isClientRectOnScreen(e = {
			...
			return a.Util.haveIntersection(n, this.getClientRect());
		}
		static create(e, t) {
			return a.Util._isString(e) && (e = JSON.parse(e)), this._createNode(e, t);
		}
		static _createNode(t, n) {
			let r = e.prototype.getClassName.call(t), o = t.children, s, c, l;
			n && (t.attrs.container = n), i.Konva[r] || (a.Util.warn("Can not find a node with class name \"" + r + "\". Fallback to \"Shape\"."), r = "Shape");
			let u = i.Konva[r];
			if (s = new u(t.attrs), o) for (c = o.length, l = 0; l < c; l++) s.add(e._createNode(o[l]));
			return s;
		}
	};
	e.Node = L, L.prototype.nodeType = "Node", L.prototype._attrsAffectingSize = [], L.prototype.eventListeners = {}, L.prototype.on.call(L.prototype, F, function() {
		if (this._batchingTransformChange) {
			this._needClearTransformCache = !0;
			return;
		}
		this._clearCache(M), this._clearSelfAndDescendantCache(l);
	...
	var R = r.Factory.addGetterSetter;
	R(L, "zIndex"), R(L, "absolutePosition"), R(L, "position"), R(L, "x", 0, (0, o.getNumberValidator)()), R(L, "y", 0, (0, o.getNumberValidator)()), R(L, "globalCompositeOperation", "source-over", (0, o.getStringValidator)()), R(L, "opacity", 1, (0, o.getNumberValidator)()), R(L, "name", "", (0, o.getStringValidator)()), R(L, "id", "", (0, o.getStringValidator)()), R(L, "rotation", 0, (0, o.getNumberValidator)()), r.Factory.addComponentsGetterSetter(L, "scale", ["x", "y"]), R(L, "scaleX", 1, (0, o.getNumberValidator)()), R(L, "scaleY", 1, (0, o.getNumberValidator)()), r.Factory.addComponentsGetterSetter(L, "skew", ["x", "y"]), R(L, "skewX", 0, (0, o.getNumberValidator)()), R(L, "skewY", 0, (0, o.getNumberValidator)()), r.Factory.addComponentsGetterSetter(L, "offset", ["x", "y"]), R(L, "offsetX", 0, (0, o.getNumberValidator)()), R(L, "offsetY", 0, (0, o.getNumberValidator)()), R(L, "dragDistance", void 0, (0, o.getNumberValidator)()), R(L, "width", 0, (0, o.getNumberValidator)()), R(L, "height", 0, (0, o.getNumberValidator)()), R(L, "listening", !0, (0, o.getBooleanValidator)()), R(L, "preventDefault", !0, (0, o.getBooleanValidator)()), R(L, "filters", void 0, function(e) {
		return this._filterUpToDate = !1, e;
	}), R(L, "visible", !0, (0, o.getBooleanValidator)()), R(L, "transformsEnabled", "all", (0, o.getStringValidator)()), R(L, "size"), R(L, "dragBoundFunc"), R(L, "draggable", !1, (0, o.getBooleanValidator)()), r.Factory.backCompat(L, {
		rotateDeg: "rotate",
	...
	var t = w(), n = T(), r = C(), i = class extends n.Node {
		constructor() {
		...
		toObject() {
			let e = n.Node.prototype.toObject.call(this);
		...
		clone(e) {
			let t = n.Node.prototype.clone.call(this, e);
	...
	e.Container = i, t.Factory.addComponentsGetterSetter(i, "clip", [
		"x",
		"y",
		"width",
		"height"
	]), t.Factory.addGetterSetter(i, "clipX", void 0, (0, r.getNumberValidator)()), t.Factory.addGetterSetter(i, "clipY", void 0, (0, r.getNumberValidator)()), t.Factory.addGetterSetter(i, "clipWidth", void 0, (0, r.getNumberValidator)()), t.Factory.addGetterSetter(i, "clipHeight", void 0, (0, r.getNumberValidator)()), t.Factory.addGetterSetter(i, "clipFunc");
})), D = /* @__PURE__ */ m(((e) => {
	Object.defineProperty(e, "__esModule", { value: !0 }), e.getCapturedShape = i, e.createEvent = a, e.hasPointerCapture = o, e.setPointerCapture = s, e.releaseCapture = c;
	var t = v(), n = /* @__PURE__ */ new Map(), r = t.Konva._global.PointerEvent !== void 0;
	function i(e) {
		return n.get(e);
	}
	function a(e) {
	...
	function s(e, t) {
		c(e), t.getStage() && (n.set(e, t), r && t._fire("gotpointercapture", a(new PointerEvent("gotpointercapture"))));
	}
	function c(e, t) {
		let i = n.get(e);
		if (!i) return;
		let o = i.getStage();
		o && o.content, n.delete(e), r && i._fire("lostpointercapture", a(new PointerEvent("lostpointercapture")));
	}
})), O = /* @__PURE__ */ m(((e) => {
	Object.defineProperty(e, "__esModule", { value: !0 }), e.Stage = e.stages = void 0;
	var t = y(), n = w(), r = E(), i = v(), a = x(), o = S(), s = v(), c = D(), l = "Stage", u = "string", d = "px", f = "mouseout", p = "mouseleave", m = "mouseover", h = "mouseenter", g = "mousemove", _ = "mousedown", b = "mouseup", C = "pointermove", T = "pointerdown", O = "pointerup", k = "pointercancel", A = "lostpointercapture", j = "pointerout", M = "pointerleave", N = "pointerover", P = "pointerenter", F = "contextmenu", I = "touchstart", L = "touchend", R = "touchmove", z = "touchcancel", ee = "wheel", te = 5, ne = [
		[h, "_pointerenter"],
		[_, "_pointerdown"],
		[g, "_pointermove"],
		[b, "_pointerup"],
	...
	}, V = (e) => e.indexOf("pointer") >= 0 ? "pointer" : e.indexOf("touch") >= 0 ? "touch" : "mouse", re = (e) => {
		let t = V(e);
		if (t === "pointer") return i.Konva.pointerEventsEnabled && B.pointer;
		if (t === "touch") return B.touch;
		if (t === "mouse") return B.mouse;
	};
	function H(e = {}) {
		return (e.clipFunc || e.clipWidth || e.clipHeight) && t.Util.warn("Stage does not support clipping. Please use clip for Layers or Groups."), e;
	}
	var U = "Pointer position is missing and not registered by the stage. Looks like it is outside of the stage container. You can set it manually from event: stage.setPointersPositions(event);";
	e.stages = [];
	var ie = class extends r.Container {
		constructor(t) {
			super(H(t)), this._pointerPositions = [], this._changedPointerPositions = [], this._buildDOM(), this._bindContentEvents(), e.stages.push(this), this.on("widthChange.konva heightChange.konva", this._resizeDOM), this.on("visibleChange.konva", this._checkVisibility), this.on("clipWidthChange.konva clipHeightChange.konva clipFuncChange.konva", () => {
				H(this.attrs);
			}), this._checkVisibility();
		}
		_validateAdd(e) {
			let n = e.getType() === "Layer", r = e.getType() === "FastLayer";
			n || r || t.Util.throw("You may only add layers to the stage.");
		}
		_checkVisibility() {
		...
		clone(e) {
			return e ||= {}, e.container = typeof document < "u" && document.createElement("div"), r.Container.prototype.clone.call(this, e);
		}
		destroy() {
			super.destroy();
			let n = this.content;
			n && t.Util._isInDocument(n) && this.container().removeChild(n);
			let r = e.stages.indexOf(this);
			return r > -1 && e.stages.splice(r, 1), t.Util.releaseCanvas(this.bufferCanvas._canvas, this.bufferHitCanvas._canvas), this;
		}
		getPointerPosition() {
		...
		_toKonvaCanvas(e) {
			e ||= {}, e.x = e.x || 0, e.y = e.y || 0, e.width = e.width || this.width(), e.height = e.height || this.height();
			let t = new a.SceneCanvas({
				width: e.width,
				height: e.height,
		...
		_resizeDOM() {
			...
			this.content && (this.content.style.width = e + d, this.content.style.height = t + d), this.bufferCanvas.setSize(e, t), this.bufferHitCanvas.setSize(e, t), this.children.forEach((n) => {
				n.setSize({
					width: e,
					height: t
				}), n.draw();
		...
		add(e, ...n) {
			...
			return r > te && t.Util.warn("The stage has " + r + " layers. Recommended maximum number of layers is 3-5. Adding more layers into the stage may drop the performance. Rethink your tree structure, you can use Konva.Group."), e.setSize({
				width: this.width(),
				height: this.height()
			}), e.draw(), i.Konva.isBrowser && this.content.appendChild(e.canvas._canvas), this;
		}
		getParent() {
			return null;
		}
		...
		hasPointerCapture(e) {
			return c.hasPointerCapture(e, this);
		}
		setPointerCapture(e) {
			c.setPointerCapture(e, this);
		}
		releaseCapture(e) {
			c.releaseCapture(e, this);
		}
		getLayers() {
			return this.children;
		}
		_bindContentEvents() {
			i.Konva.isBrowser && ne.forEach(([e, t]) => {
		...
		_pointerleave(e) {
			let t = re(e.type), n = V(e.type);
			if (!t) return;
			this.setPointersPositions(e);
			let r = this._getTargetShape(n), a = !(i.Konva.isDragging() || i.Konva.isTransforming()) || i.Konva.hitOnDragEnabled;
			r && a ? (r._fireAndBubble(t.pointerout, { evt: e }), r._fireAndBubble(t.pointerleave, { evt: e }), this._fire(t.pointerleave, {
				evt: e,
				target: this,
				currentTarget: this
			}), this[n + "targetShape"] = null) : a && (this._fire(t.pointerleave, {
				evt: e,
				target: this,
				currentTarget: this
		...
		_pointerdown(e) {
			...
			this._changedPointerPositions.forEach((a) => {
				let s = this.getIntersection(a);
				if (o.DD.justDragged = !1, i.Konva["_" + n + "ListenClick"] = !0, !s || !s.isListening()) {
					this[n + "ClickStartShape"] = void 0;
					return;
				}
				i.Konva.capturePointerEventsEnabled && s.setPointerCapture(a.id), this[n + "ClickStartShape"] = s, s._fireAndBubble(t.pointerdown, {
					evt: e,
					pointerId: a.id
				}), r = !0;
				let c = e.type.indexOf("touch") >= 0;
				s.preventDefault() && e.cancelable && c && e.preventDefault();
		...
		_pointermove(e) {
			let t = re(e.type), n = V(e.type);
			if (!t || (i.Konva.isDragging() && o.DD.node.preventDefault() && e.cancelable && e.preventDefault(), this.setPointersPositions(e), !(!(i.Konva.isDragging() || i.Konva.isTransforming()) || i.Konva.hitOnDragEnabled))) return;
			let r = {}, a = !1, s = this._getTargetShape(n);
			this._changedPointerPositions.forEach((i) => {
				let o = c.getCapturedShape(i.id) || this.getIntersection(i), l = i.id, u = {
					evt: e,
					pointerId: l
				}, d = s !== o;
				if (d && s && (s._fireAndBubble(t.pointerout, { ...u }, o), s._fireAndBubble(t.pointerleave, { ...u }, o)), o) {
					if (r[o._id]) return;
					r[o._id] = !0;
				}
				o && o.isListening() ? (a = !0, d && (o._fireAndBubble(t.pointerover, { ...u }, s), o._fireAndBubble(t.pointerenter, { ...u }, s), this[n + "targetShape"] = o), o._fireAndBubble(t.pointermove, { ...u })) : s && (this._fire(t.pointerover, {
					evt: e,
					target: this,
					currentTarget: this,
					pointerId: l
		...
		_pointerup(e) {
			...
			if (!t) return;
			this.setPointersPositions(e);
			let r = this[n + "ClickStartShape"], a = this[n + "ClickEndShape"], s = {}, l = !1;
			this._changedPointerPositions.forEach((u) => {
				let d = c.getCapturedShape(u.id) || this.getIntersection(u);
				if (d) {
					if (d.releaseCapture(u.id), s[d._id]) return;
					s[d._id] = !0;
				}
				let f = u.id, p = {
					evt: e,
				...
				i.Konva["_" + n + "InDblClickWindow"] ? (m = !0, clearTimeout(this[n + "DblTimeout"])) : o.DD.justDragged || (i.Konva["_" + n + "InDblClickWindow"] = !0, clearTimeout(this[n + "DblTimeout"])), this[n + "DblTimeout"] = setTimeout(function() {
					i.Konva["_" + n + "InDblClickWindow"] = !1;
				}, i.Konva.dblClickWindow), d && d.isListening() ? (l = !0, this[n + "ClickEndShape"] = d, d._fireAndBubble(t.pointerup, { ...p }), i.Konva["_" + n + "ListenClick"] && r && r === d && (d._fireAndBubble(t.pointerclick, { ...p }), m && a && a === d && d._fireAndBubble(t.pointerdblclick, { ...p }))) : (this[n + "ClickEndShape"] = null, i.Konva["_" + n + "ListenClick"] && this._fire(t.pointerclick, {
					evt: e,
					target: this,
					currentTarget: this,
					pointerId: f
				}), m && this._fire(t.pointerdblclick, {
					evt: e,
					target: this,
					currentTarget: this,
					pointerId: f
				}));
			}), l || this._fire(t.pointerup, {
				evt: e,
				target: this,
				currentTarget: this,
				pointerId: this._changedPointerPositions[0].id
			}), i.Konva["_" + n + "ListenClick"] = !1, e.cancelable && n !== "touch" && n !== "pointer" && e.preventDefault();
		}
		_contextmenu(e) {
			this.setPointersPositions(e);
			let t = this.getIntersection(this.getPointerPosition());
			t && t.isListening() ? t._fireAndBubble(F, { evt: e }) : this._fire(F, {
				evt: e,
				target: this,
				currentTarget: this
			});
		...
		_wheel(e) {
			this.setPointersPositions(e);
			let t = this.getIntersection(this.getPointerPosition());
			t && t.isListening() ? t._fireAndBubble(ee, { evt: e }) : this._fire(ee, {
				evt: e,
				target: this,
				currentTarget: this
			});
		...
		_pointercancel(e) {
			this.setPointersPositions(e);
			let t = c.getCapturedShape(e.pointerId) || this.getIntersection(this.getPointerPosition());
			t && t._fireAndBubble(O, c.createEvent(e)), c.releaseCapture(e.pointerId);
		}
		_lostpointercapture(e) {
			c.releaseCapture(e.pointerId);
		}
		setPointersPositions(e) {
			let n = this._getContentPosition(), r = null, i = null;
			e ||= window.event, e.touches === void 0 ? (r = (e.clientX - n.left) / n.scaleX, i = (e.clientY - n.top) / n.scaleY, this.pointerPos = {
				x: r,
				y: i
			}, this._pointerPositions = [{
				x: r,
				y: i,
				id: t.Util._getFirstPointerId(e)
				...
				this._pointerPositions.push({
					id: e.identifier,
					x: (e.clientX - n.left) / n.scaleX,
					y: (e.clientY - n.top) / n.scaleY
				});
	...
	e.Stage = ie, ie.prototype.nodeType = l, (0, s._registerNode)(ie), n.Factory.addGetterSetter(ie, "container"), i.Konva.isBrowser && document.addEventListener("visibilitychange", () => {
		e.stages.forEach((e) => {
			e.batchDraw();
		});
	});
	...
	var t = v(), n = y(), r = w(), i = T(), a = C(), o = v(), s = D(), c = "hasShadow", l = "shadowRGBA", u = "patternImage", d = "linearGradient", f = "radialGradient", p;
	function m() {
		return p || (p = n.Util.createCanvasElement().getContext("2d"), p);
	}
	e.shapes = {};
	function h(e) {
	...
	var A = class extends i.Node {
		constructor(t) {
			super(t);
			let r;
			for (; r = n.Util.getRandomColor(), !(r && !(r in e.shapes)););
		...
		__getFillPattern() {
			if (this.fillPatternImage()) {
				let e = m().createPattern(this.fillPatternImage(), this.fillPatternRepeat() || "repeat");
				if (e && e.setTransform) {
					let r = new n.Transform();
					r.translate(this.fillPatternX(), this.fillPatternY()), r.rotate(t.Konva.getAngle(this.fillPatternRotation())), r.scale(this.fillPatternScaleX(), this.fillPatternScaleY()), r.translate(-1 * this.fillPatternOffsetX(), -1 * this.fillPatternOffsetY());
					let i = r.getMatrix(), a = typeof DOMMatrix > "u" ? {
						a: i[0],
						b: i[1],
						c: i[2],
						d: i[3],
		...
		__getLinearGradient() {
			...
			if (e) {
				let t = m(), n = this.fillLinearGradientStartPoint(), r = this.fillLinearGradientEndPoint(), i = t.createLinearGradient(n.x, n.y, r.x, r.y);
		...
		__getRadialGradient() {
			...
			if (e) {
				let t = m(), n = this.fillRadialGradientStartPoint(), r = this.fillRadialGradientEndPoint(), i = t.createRadialGradient(n.x, n.y, this.fillRadialGradientStartRadius(), r.x, r.y, this.fillRadialGradientEndRadius());
				for (let t = 0; t < e.length; t += 2) i.addColorStop(e[t], e[t + 1]);
				return i;
			}
		}
		...
		_getShadowRGBA() {
			...
			let e = n.Util.colorToRGBA(this.shadowColor());
		...
		setStrokeHitEnabled(e) {
			n.Util.warn("strokeHitEnabled property is deprecated. Please use hitStrokeWidth instead."), e ? this.hitStrokeWidth("auto") : this.hitStrokeWidth(0);
		}
		getStrokeHitEnabled() {
		...
		drawScene(e, t, n) {
			...
			if (i.save(), this._useBufferCanvas()) {
				...
				r.clear(), r.save(), r._applyLineJoin(this);
				let a = this.getAbsoluteTransform(t).getMatrix();
				r.transform(a[0], a[1], a[2], a[3], a[4], a[5]), o.call(this, r, this), r.restore();
				let l = e.pixelRatio;
				s && i._applyShadow(this), i._applyOpacity(this), i._applyGlobalCompositeOperation(this), i.drawImage(e._canvas, e.x || 0, e.y || 0, e.width / l, e.height / l);
			} else {
				if (i._applyLineJoin(this), !l) {
					let e = this.getAbsoluteTransform(t).getMatrix();
		...
		drawHit(e, t, r = !1) {
			if (!this.shouldDrawHit(t, r)) return this;
			let i = this.getLayer(), a = e || i.hitCanvas, o = a && a.getContext(), s = this.hitFunc() || this.sceneFunc(), c = this._getCanvasCache(), l = c && c.hit;
			if (this.colorKey || n.Util.warn("Looks like your canvas has a destroyed shape in it. Do not reuse shape after you destroyed it. If you want to reuse shape you should call remove() instead of destroy()"), l) {
				o.save();
				let e = this.getAbsoluteTransform(t).getMatrix();
				return o.transform(e[0], e[1], e[2], e[3], e[4], e[5]), this._drawCachedHitCanvas(o), o.restore(), this;
			}
			if (!s) return this;
			if (o.save(), o._applyLineJoin(this), this !== t) {
				let e = this.getAbsoluteTransform(t).getMatrix();
				o.transform(e[0], e[1], e[2], e[3], e[4], e[5]);
			}
			return s.call(this, o, this), o.restore(), this;
		}
		drawHitFromCache(e = 0) {
			let t = this._getCanvasCache(), r = this._getCachedSceneCanvas(), i = t.hit, a = i.getContext(), o = i.getWidth(), s = i.getHeight();
			a.clear(), a.drawImage(r._canvas, 0, 0, o, s);
			try {
				let t = a.getImageData(0, 0, o, s), r = t.data, i = r.length, c = n.Util._hexToRgb(this.colorKey);
				for (let t = 0; t < i; t += 4) r[t + 3] > e ? (r[t] = c.r, r[t + 1] = c.g, r[t + 2] = c.b, r[t + 3] = 255) : r[t + 3] = 0;
				a.putImageData(t, 0, 0);
			} catch (e) {
				n.Util.error("Unable to draw hit graph from cached scene canvas. " + e.message);
			}
			return this;
		}
		hasPointerCapture(e) {
	...
	var t = y(), n = E(), r = T(), i = w(), a = x(), o = C(), s = k(), c = v(), l = "#", u = "beforeDraw", d = "draw", f = [
		{
			x: 0,
			y: 0
		},
	...
	], p = f.length, m = class extends n.Container {
		constructor(e) {
			super(e), this.canvas = new a.SceneCanvas(), this.hitCanvas = new a.HitCanvas({ pixelRatio: 1 }), this._waitingForDraw = !1, this.on("visibleChange.konva", this._checkVisibility), this._checkVisibility(), this.on("imageSmoothingEnabledChange.konva", this._setSmoothEnabled), this._setSmoothEnabled();
		}
		createPNGStream() {
		...
		moveUp() {
			if (!r.Node.prototype.moveUp.call(this)) return !1;
			let e = this.getStage();
		...
		remove() {
			...
			return r.Node.prototype.remove.call(this), e && e.parentNode && t.Util._isInDocument(e) && e.parentNode.removeChild(e), this;
		}
		getStage() {
			return this.parent;
		}
		...
		_validateAdd(e) {
			let n = e.getType();
			n !== "Group" && n !== "Shape" && t.Util.throw("You may only add groups and shapes to a layer.");
		}
		_toKonvaCanvas(e) {
			return e ||= {}, e.width = e.width || this.getWidth(), e.height = e.height || this.getHeight(), e.x = e.x === void 0 ? this.x() : e.x, e.y = e.y === void 0 ? this.y() : e.y, r.Node.prototype._toKonvaCanvas.call(this, e);
		}
		_checkVisibility() {
		...
		getIntersection(e) {
			...
			for (;;) {
				for (let r = 0; r < p; r++) {
					let i = f[r], a = this._getIntersection({
						x: e.x + i.x * t,
						y: e.y + i.y * t
					}), o = a.shape;
					if (o) return o;
		...
		_getIntersection(e) {
			let n = this.hitCanvas.pixelRatio, r = this.hitCanvas.context.getImageData(Math.round(e.x * n), Math.round(e.y * n), 1, 1).data, i = r[3];
			if (i === 255) {
				let e = t.Util._rgbToHex(r[0], r[1], r[2]), n = s.shapes[l + e];
				return n ? { shape: n } : { antialiased: !0 };
			} else if (i > 0) return { antialiased: !0 };
			return {};
		}
		drawScene(e, t, r) {
			let i = this.getLayer(), a = e || i && i.getCanvas();
			return this._fire(u, { node: this }), this.clearBeforeDraw() && a.getContext().clear(), n.Container.prototype.drawScene.call(this, a, t, r), this._fire(d, { node: this }), this;
		}
		drawHit(e, t) {
			let r = this.getLayer(), i = e || r && r.hitCanvas;
			return r && r.clearBeforeDraw() && r.getHitCanvas().getContext().clear(), n.Container.prototype.drawHit.call(this, i, t), this;
		}
		enableHitGraph() {
		...
		setHitGraphEnabled(e) {
			t.Util.warn("hitGraphEnabled method is deprecated. Please use layer.listening() instead."), this.listening(e);
		}
	...
	e.Layer = m, m.prototype.nodeType = "Layer", (0, c._registerNode)(m), i.Factory.addGetterSetter(m, "imageSmoothingEnabled", !0), i.Factory.addGetterSetter(m, "clearBeforeDraw", !0), i.Factory.addGetterSetter(m, "hitGraphEnabled", !0, (0, o.getBooleanValidator)());
})), j = /* @__PURE__ */ m(((e) => {
	Object.defineProperty(e, "__esModule", { value: !0 }), e.FastLayer = void 0;
	var t = y(), n = A(), r = v(), i = class extends n.Layer {
		constructor(e) {
			super(e), this.listening(!1), t.Util.warn("Konva.Fast layer is deprecated. Please use \"new Konva.Layer({ listening: false })\" instead.");
		}
	};
	e.FastLayer = i, i.prototype.nodeType = "FastLayer", (0, r._registerNode)(i);
})), M = /* @__PURE__ */ m(((e) => {
	Object.defineProperty(e, "__esModule", { value: !0 }), e.Group = void 0;
	var t = y(), n = E(), r = v(), i = class extends n.Container {
		_validateAdd(e) {
			let n = e.getType();
			n !== "Group" && n !== "Shape" && t.Util.throw("You may only add groups and shapes to groups.");
		}
	};
	e.Group = i, i.prototype.nodeType = "Group", (0, r._registerNode)(i);
})), N = /* @__PURE__ */ m(((e) => {
	Object.defineProperty(e, "__esModule", { value: !0 }), e.Animation = void 0;
	var t = v(), n = y(), r = (function() {
		...
		start() {
			return this.stop(), this.frame.timeDiff = 0, this.frame.lastTime = r(), e._addAnimation(this), this;
		}
		stop() {
		...
		static _runFrames() {
			...
			for (let n = 0; n < t.length; n++) {
				let i = t[n], a = i.layers, o = i.func;
				i._updateFrameObject(r());
				let s = a.length, c;
				if (c = o ? o.call(i, i.frame) !== !1 : !0, c) for (let t = 0; t < s; t++) {
					let n = a[t];
		...
		static _animationLoop() {
			let t = e;
			t.animations.length ? (t._runFrames(), n.Util.requestAnimFrame(t._animationLoop)) : t.animRunning = !1;
		}
		static _handleAnimation() {
			this.animRunning || (this.animRunning = !0, n.Util.requestAnimFrame(this._animationLoop));
		}
	};
	e.Animation = i, i.animations = [], i.animIdCounter = 0, i.animRunning = !1;
})), P = /* @__PURE__ */ m(((e) => {
	Object.defineProperty(e, "__esModule", { value: !0 }), e.Easings = e.Tween = void 0;
	var t = y(), n = N(), r = T(), i = v(), a = {
		node: 1,
		duration: 1,
		easing: 1,
		onFinish: 1,
		...
		onEnterFrame() {
			...
			this.state === s ? this.setTime(e) : this.state === c && this.setTime(this.duration - e);
		}
		pause() {
		...
		constructor(o) {
			let s = this, c = o.node, l = c._id, f = o.easing || e.Easings.Linear, p = !!o.yoyo, m, h;
			m = o.duration === void 0 ? .3 : o.duration === 0 ? .001 : o.duration, this.node = c, this._id = u++;
			let g = c.getLayer() || (c instanceof i.Konva.Stage ? c.getLayers() : null);
			for (h in g || t.Util.error("Tween constructor have `node` that is not in a layer. Please add node into layer first."), this.anim = new n.Animation(function() {
				s.tween.onEnterFrame();
			}, g), this.tween = new d(h, function(e) {
				s._tweenFunc(e);
			}, f, 0, 1, m * 1e3, p), this._addListeners(), r.attrs[l] || (r.attrs[l] = {}), r.attrs[l][this._id] || (r.attrs[l][this._id] = {}), r.tweens[l] || (r.tweens[l] = {}), o) a[h] === void 0 && this._addAttr(h, o[h]);
			this.reset(), this.onFinish = o.onFinish, this.onReset = o.onReset, this.onUpdate = o.onUpdate;
		}
		_addAttr(e, n) {
			let i = this.node, a = i._id, o, s, c, u, d, f = r.tweens[a][e];
			f && delete r.attrs[a][f][e];
			let p = i.getAttr(e);
			if (t.Util._isArray(n)) if (o = [], s = Math.max(n.length, p.length), e === "points" && n.length !== p.length && (n.length > p.length ? (u = p, p = t.Util._prepareArrayForTween(p, n, i.closed())) : (c = n, n = t.Util._prepareArrayForTween(n, p, i.closed()))), e.indexOf("fill") === 0) for (let e = 0; e < s; e++) if (e % 2 == 0) o.push(n[e] - p[e]);
			else {
				let r = t.Util.colorToRGBA(p[e]);
				d = t.Util.colorToRGBA(n[e]), p[e] = r, o.push({
					r: d.r - r.r,
					g: d.g - r.g,
					b: d.b - r.b,
					a: d.a - r.a
			...
			else for (let e = 0; e < s; e++) o.push(n[e] - p[e]);
			else l.indexOf(e) === -1 ? o = n - p : (p = t.Util.colorToRGBA(p), d = t.Util.colorToRGBA(n), o = {
				r: d.r - p.r,
				g: d.g - p.g,
				b: d.b - p.b,
				a: d.a - p.a
			});
			r.attrs[a][this._id][e] = {
				start: p,
				diff: o,
				end: n,
				trueEnd: c,
				trueStart: u
			}, r.tweens[a][e] = this._id;
		}
		_tweenFunc(e) {
			let n = this.node, i = r.attrs[n._id][this._id], a, o, s, c, u, d, f, p;
			for (a in i) {
				if (o = i[a], s = o.start, c = o.diff, p = o.end, t.Util._isArray(s)) if (u = [], f = Math.max(s.length, p.length), a.indexOf("fill") === 0) for (d = 0; d < f; d++) d % 2 == 0 ? u.push((s[d] || 0) + c[d] * e) : u.push("rgba(" + Math.round(s[d].r + c[d].r * e) + "," + Math.round(s[d].g + c[d].g * e) + "," + Math.round(s[d].b + c[d].b * e) + "," + (s[d].a + c[d].a * e) + ")");
				else for (d = 0; d < f; d++) u.push((s[d] || 0) + c[d] * e);
				else u = l.indexOf(a) === -1 ? s + c * e : "rgba(" + Math.round(s.r + c.r * e) + "," + Math.round(s.g + c.g * e) + "," + Math.round(s.b + c.b * e) + "," + (s.a + c.a * e) + ")";
				n.setAttr(a, u);
			}
		}
		_addListeners() {
			this.tween.onPlay = () => {
				...
				let e = this.node, t = r.attrs[e._id][this._id];
		...
		destroy() {
			let e = this.node._id, t = this._id, n = r.tweens[e];
			this.pause(), this.anim && this.anim.stop();
			for (let t in n) delete r.tweens[e][t];
			delete r.attrs[e][t], r.tweens[e] && (Object.keys(r.tweens[e]).length === 0 && delete r.tweens[e], Object.keys(r.attrs[e]).length === 0 && delete r.attrs[e]);
		}
	};
	e.Tween = f, f.attrs = {}, f.tweens = {}, r.Node.prototype.to = function(e) {
		let t = e.onFinish;
		...
		BounceEaseIn(t, n, r, i) {
			return r - e.Easings.BounceEaseOut(i - t, 0, r, i) + n;
		},
		BounceEaseInOut(t, n, r, i) {
			return t < i / 2 ? e.Easings.BounceEaseIn(t * 2, 0, r, i) * .5 + n : e.Easings.BounceEaseOut(t * 2 - i, 0, r, i) * .5 + r * .5 + n;
		},
		EaseIn(e, t, n, r) {
			return n * (e /= r) * e + t;
		},
	...
	var t = v(), n = y(), r = T(), i = E(), a = O(), o = A(), s = j(), c = M(), l = S(), u = k(), d = N(), f = P(), p = b(), m = x();
	...
	var t = w(), n = k(), r = v(), i = C(), a = v(), o = class extends n.Shape {
		_sceneFunc(e) {
			let t = r.Konva.getAngle(this.angle()), n = this.clockwise();
		...
		getSelfRect() {
			let e = this.innerRadius(), t = this.outerRadius(), n = this.clockwise(), i = r.Konva.getAngle(n ? 360 - this.angle() : this.angle()), a = Math.cos(Math.min(i, Math.PI)), o = Math.sin(Math.min(Math.max(Math.PI, i), 3 * Math.PI / 2)), s = Math.sin(Math.min(i, Math.PI / 2)), c = a * (a > 0 ? e : t), l = 1 * t, u = o * (o > 0 ? e : t), d = s * (s > 0 ? t : e);
			return {
				x: c,
				y: n ? -1 * d : u,
				width: l - c,
	...
	e.Arc = o, o.prototype._centroid = !0, o.prototype.className = "Arc", o.prototype._attrsAffectingSize = [
		"innerRadius",
		"outerRadius",
		"angle",
		"clockwise"
	], (0, a._registerNode)(o), t.Factory.addGetterSetter(o, "innerRadius", 0, (0, i.getNumberValidator)()), t.Factory.addGetterSetter(o, "outerRadius", 0, (0, i.getNumberValidator)()), t.Factory.addGetterSetter(o, "angle", 0, (0, i.getNumberValidator)()), t.Factory.addGetterSetter(o, "clockwise", !1, (0, i.getBooleanValidator)());
})), L = /* @__PURE__ */ m(((e) => {
	...
	var s = class extends r.Shape {
		...
		_getTensionPointsClosed() {
			let e = this.points(), t = e.length, n = this.tension(), r = a(e[t - 2], e[t - 1], e[0], e[1], e[2], e[3], n), i = a(e[t - 4], e[t - 3], e[t - 2], e[t - 1], e[0], e[1], n), s = o(e, n);
			return [r[2], r[3]].concat(s, [
				i[0],
				i[1],
				e[t - 2],
				e[t - 1],
	...
	e.Line = s, s.prototype.className = "Line", s.prototype._attrsAffectingSize = [
		"points",
		"bezier",
		"tension"
	], (0, n._registerNode)(s), t.Factory.addGetterSetter(s, "closed", !1), t.Factory.addGetterSetter(s, "bezier", !1), t.Factory.addGetterSetter(s, "tension", 0, (0, i.getNumberValidator)()), t.Factory.addGetterSetter(s, "points", [], (0, i.getNumberArrayValidator)());
})), R = /* @__PURE__ */ m(((e) => {
	...
	], e.getCubicArcLength = (n, r, i) => {
		let a, o, s = i / 2;
		a = 0;
		for (let i = 0; i < 20; i++) o = s * e.tValues[20][i] + s, a += e.cValues[20][i] * t(n, r, o);
		return s * a;
	}, e.getQuadraticArcLength = (e, t, n) => {
		n === void 0 && (n = 1);
	...
	function t(e, t, r) {
		let i = n(1, r, e), a = n(1, r, t), o = i * i + a * a;
		return Math.sqrt(o);
	}
	var n = (t, r, i) => {
		let a = i.length - 1, o, s;
		if (a === 0) return 0;
		if (t === 0) {
			s = 0;
			for (let t = 0; t <= a; t++) s += e.binomialCoefficients[a][t] * (1 - r) ** (a - t) * r ** +t * i[t];
			return s;
		} else {
			o = Array(a);
			for (let e = 0; e < a; e++) o[e] = a * (i[e + 1] - i[e]);
			return n(t - 1, r, o);
		}
	};
	e.t2length = (e, t, n) => {
	...
	var t = w(), n = v(), r = k(), i = R(), a = class e extends r.Shape {
		constructor(e) {
		...
		static getPointAtLengthOfDataArray(t, n) {
			let r, a = 0, o = n.length;
			if (!o) return null;
			for (; a < o && t > n[a].pathLength;) t -= n[a].pathLength, ++a;
			if (a === o) return r = n[a - 1].points.slice(-2), {
				x: r[0],
				y: r[1]
			};
			if (t < .01) return n[a].command === "M" ? (r = n[a].points.slice(0, 2), {
				x: r[0],
				y: r[1]
			}) : {
				x: n[a].start.x,
			...
			let s = n[a], c = s.points;
			switch (s.command) {
				case "L": return e.getPointOnLine(t, s.start.x, s.start.y, c[0], c[1]);
				case "C": return e.getPointOnCubicBezier((0, i.t2length)(t, e.getPathLength(n), (e) => (0, i.getCubicArcLength)([
					s.start.x,
					c[0],
					c[2],
					c[4]
					...
					c[3],
					c[5]
				], e)), s.start.x, s.start.y, c[0], c[1], c[2], c[3], c[4], c[5]);
				case "Q": return e.getPointOnQuadraticBezier((0, i.t2length)(t, e.getPathLength(n), (e) => (0, i.getQuadraticArcLength)([
					s.start.x,
					c[0],
					c[2]
				], [
					...
					c[1],
					c[3]
				], e)), s.start.x, s.start.y, c[0], c[1], c[2], c[3]);
				case "A":
					let r = c[0], a = c[1], o = c[2], l = c[3], u = c[5], d = c[6], f = c[4];
					return f += u * t / s.pathLength, e.getPointOnEllipticalArc(r, a, o, l, f, d);
			}
			return null;
		}
		...
		static calcLength(t, n, r, a) {
			let o, s, c, l, u = e;
			switch (r) {
				case "L": return u.getLineLength(t, n, a[0], a[1]);
				case "C": return (0, i.getCubicArcLength)([
					t,
					a[0],
					a[2],
					a[4]
				...
				case "Q": return (0, i.getQuadraticArcLength)([
					t,
					a[0],
					a[2]
				], [
				...
				case "A":
					o = 0;
					let e = a[4], r = a[5], d = a[4] + r, f = Math.PI / 180;
					if (Math.abs(e - d) < f && (f = Math.abs(e - d)), s = u.getPointOnEllipticalArc(a[0], a[1], a[2], a[3], e, 0), r < 0) for (l = e - f; l > d; l -= f) c = u.getPointOnEllipticalArc(a[0], a[1], a[2], a[3], l, 0), o += u.getLineLength(s.x, s.y, c.x, c.y), s = c;
					else for (l = e + f; l < d; l += f) c = u.getPointOnEllipticalArc(a[0], a[1], a[2], a[3], l, 0), o += u.getLineLength(s.x, s.y, c.x, c.y), s = c;
					return c = u.getPointOnEllipticalArc(a[0], a[1], a[2], a[3], d, 0), o += u.getLineLength(s.x, s.y, c.x, c.y), o;
			}
			return 0;
		}
		static convertEndpointToCenterParameterization(e, t, n, r, i, a, o, s, c) {
			...
			}, x = b([1, 0], [(u - m) / o, (d - h) / s]), S = [(u - m) / o, (d - h) / s], C = [(-1 * u - m) / o, (-1 * d - h) / s], w = b(S, C);
			return y(S, C) <= -1 && (w = Math.PI), y(S, C) >= 1 && (w = 0), a === 0 && w > 0 && (w -= 2 * Math.PI), a === 1 && w < 0 && (w += 2 * Math.PI), [
				g,
				_,
				o,
				s,
	...
	e.Path = a, a.prototype.className = "Path", a.prototype._attrsAffectingSize = ["data"], (0, n._registerNode)(a), t.Factory.addGetterSetter(a, "data");
})), ee = /* @__PURE__ */ m(((e) => {
	Object.defineProperty(e, "__esModule", { value: !0 }), e.Arrow = void 0;
	var t = w(), n = L(), r = C(), i = v(), a = z(), o = class extends n.Line {
		_sceneFunc(e) {
			super._sceneFunc(e);
			let t = Math.PI * 2, n = this.points(), r = n, i = this.tension() !== 0 && n.length > 4;
			i && (r = this.getTensionPoints());
			let o = this.pointerLength(), s = n.length, c, l;
			if (i) {
				let e = [
					r[r.length - 4],
					r[r.length - 3],
					...
					n[s - 2],
					n[s - 1]
				], t = a.Path.calcLength(r[r.length - 4], r[r.length - 3], "C", e), i = a.Path.getPointOnQuadraticBezier(Math.min(1, 1 - o / t), e[0], e[1], e[2], e[3], e[4], e[5]);
				c = n[s - 2] - i.x, l = n[s - 1] - i.y;
			} else c = n[s - 2] - n[s - 4], l = n[s - 1] - n[s - 3];
			let u = (Math.atan2(l, c) + t) % t, d = this.pointerWidth();
			this.pointerAtEnding() && (e.save(), e.beginPath(), e.translate(n[s - 2], n[s - 1]), e.rotate(u), e.moveTo(0, 0), e.lineTo(-o, d / 2), e.lineTo(-o, -d / 2), e.closePath(), e.restore(), this.__fillStroke(e)), this.pointerAtBeginning() && (e.save(), e.beginPath(), e.translate(n[0], n[1]), i ? (c = (r[0] + r[2]) / 2 - n[0], l = (r[1] + r[3]) / 2 - n[1]) : (c = n[2] - n[0], l = n[3] - n[1]), e.rotate((Math.atan2(-l, -c) + t) % t), e.moveTo(0, 0), e.lineTo(-o, d / 2), e.lineTo(-o, -d / 2), e.closePath(), e.restore(), this.__fillStroke(e));
		}
		__fillStroke(e) {
	...
	e.Arrow = o, o.prototype.className = "Arrow", (0, i._registerNode)(o), t.Factory.addGetterSetter(o, "pointerLength", 10, (0, r.getNumberValidator)()), t.Factory.addGetterSetter(o, "pointerWidth", 10, (0, r.getNumberValidator)()), t.Factory.addGetterSetter(o, "pointerAtBeginning", !1), t.Factory.addGetterSetter(o, "pointerAtEnding", !0);
})), te = /* @__PURE__ */ m(((e) => {
	Object.defineProperty(e, "__esModule", { value: !0 }), e.Circle = void 0;
	var t = w(), n = k(), r = C(), i = v(), a = class extends n.Shape {
		_sceneFunc(e) {
	...
	e.Circle = a, a.prototype._centroid = !0, a.prototype.className = "Circle", a.prototype._attrsAffectingSize = ["radius"], (0, i._registerNode)(a), t.Factory.addGetterSetter(a, "radius", 0, (0, r.getNumberValidator)());
})), ne = /* @__PURE__ */ m(((e) => {
	...
	var t = y(), n = w(), r = k(), i = v(), a = C(), o = class e extends r.Shape {
		...
		_sceneFunc(e) {
			let n = this.getWidth(), r = this.getHeight(), i = this.cornerRadius(), a = this.attrs.image, o;
			if (a) {
				let e = this.attrs.cropWidth, t = this.attrs.cropHeight;
				o = e && t ? [
					a,
					this.cropX(),
					this.cropY(),
			...
			(this.hasFill() || this.hasStroke() || i) && (e.beginPath(), i ? t.Util.drawRoundedRectPath(e, n, r, i) : e.rect(0, 0, n, r), e.closePath(), e.fillStrokeShape(this)), a && (i && e.clip(), e.drawImage.apply(e, o));
		}
		_hitFunc(e) {
			let n = this.width(), r = this.height(), i = this.cornerRadius();
			e.beginPath(), i ? t.Util.drawRoundedRectPath(e, n, r, i) : e.rect(0, 0, n, r), e.closePath(), e.fillStrokeShape(this);
		}
		getWidth() {
		...
		static fromURL(n, r, i = null) {
			let a = t.Util.createImageElement();
			a.onload = function() {
				r(new e({ image: a }));
			}, a.onerror = i, a.crossOrigin = "Anonymous", a.src = n;
		}
	};
	e.Image = o, o.prototype.className = "Image", (0, i._registerNode)(o), n.Factory.addGetterSetter(o, "cornerRadius", 0, (0, a.getNumberOrArrayOfNumbersValidator)(4)), n.Factory.addGetterSetter(o, "image"), n.Factory.addComponentsGetterSetter(o, "crop", [
		"x",
		"y",
		"width",
		"height"
	]), n.Factory.addGetterSetter(o, "cropX", 0, (0, a.getNumberValidator)()), n.Factory.addGetterSetter(o, "cropY", 0, (0, a.getNumberValidator)()), n.Factory.addGetterSetter(o, "cropWidth", 0, (0, a.getNumberValidator)()), n.Factory.addGetterSetter(o, "cropHeight", 0, (0, a.getNumberValidator)());
})), V = /* @__PURE__ */ m(((e) => {
	Object.defineProperty(e, "__esModule", { value: !0 }), e.Tag = e.Label = void 0;
	var t = w(), n = k(), r = M(), i = C(), a = v(), o = [
		"fontFamily",
		"fontSize",
		"fontStyle",
		"padding",
		...
		"pointerWidth",
		"pointerHeight"
	], s = "Change.konva", c = "none", l = "up", u = "right", d = "down", f = "left", p = o.length, m = class extends r.Group {
		constructor(e) {
	...
	e.Label = m, m.prototype.className = "Label", (0, a._registerNode)(m);
	var h = class extends n.Shape {
		_sceneFunc(e) {
			let t = this.width(), n = this.height(), r = this.pointerDirection(), i = this.pointerWidth(), a = this.pointerHeight(), o = this.cornerRadius(), s = 0, c = 0, p = 0, m = 0;
			typeof o == "number" ? s = c = p = m = Math.min(o, t / 2, n / 2) : (s = Math.min(o[0] || 0, t / 2, n / 2), c = Math.min(o[1] || 0, t / 2, n / 2), m = Math.min(o[2] || 0, t / 2, n / 2), p = Math.min(o[3] || 0, t / 2, n / 2)), e.beginPath(), e.moveTo(s, 0), r === l && (e.lineTo((t - i) / 2, 0), e.lineTo(t / 2, -1 * a), e.lineTo((t + i) / 2, 0)), e.lineTo(t - c, 0), e.arc(t - c, c, c, Math.PI * 3 / 2, 0, !1), r === u && (e.lineTo(t, (n - a) / 2), e.lineTo(t + i, n / 2), e.lineTo(t, (n + a) / 2)), e.lineTo(t, n - m), e.arc(t - m, n - m, m, 0, Math.PI / 2, !1), r === d && (e.lineTo((t + i) / 2, n), e.lineTo(t / 2, n + a), e.lineTo((t - i) / 2, n)), e.lineTo(p, n), e.arc(p, n - p, p, Math.PI / 2, Math.PI, !1), r === f && (e.lineTo(0, (n + a) / 2), e.lineTo(-1 * i, n / 2), e.lineTo(0, (n - a) / 2)), e.lineTo(0, s), e.arc(s, s, s, Math.PI, Math.PI * 3 / 2, !1), e.closePath(), e.fillStrokeShape(this);
		}
		getSelfRect() {
			let e = 0, t = 0, n = this.pointerWidth(), r = this.pointerHeight(), i = this.pointerDirection(), a = this.width(), o = this.height();
			return i === l ? (t -= r, o += r) : i === d ? o += r : i === f ? (e -= n * 1.5, a += n) : i === u && (a += n * 1.5), {
				x: e,
				y: t,
				width: a,
				height: o
	...
	e.Tag = h, h.prototype.className = "Tag", (0, a._registerNode)(h), t.Factory.addGetterSetter(h, "pointerDirection", c), t.Factory.addGetterSetter(h, "pointerWidth", 0, (0, i.getNumberValidator)()), t.Factory.addGetterSetter(h, "pointerHeight", 0, (0, i.getNumberValidator)()), t.Factory.addGetterSetter(h, "cornerRadius", 0, (0, i.getNumberOrArrayOfNumbersValidator)(4));
})), re = /* @__PURE__ */ m(((e) => {
	Object.defineProperty(e, "__esModule", { value: !0 }), e.Rect = void 0;
	var t = w(), n = k(), r = v(), i = y(), a = C(), o = class extends n.Shape {
		_sceneFunc(e) {
			let t = this.cornerRadius(), n = this.width(), r = this.height();
			e.beginPath(), t ? i.Util.drawRoundedRectPath(e, n, r, t) : e.rect(0, 0, n, r), e.closePath(), e.fillStrokeShape(this);
		}
	};
	e.Rect = o, o.prototype.className = "Rect", (0, r._registerNode)(o), t.Factory.addGetterSetter(o, "cornerRadius", 0, (0, a.getNumberOrArrayOfNumbersValidator)(4));
})), H = /* @__PURE__ */ m(((e) => {
	...
	var t = w(), n = k(), r = C(), i = v(), a = class extends n.Shape {
		...
		getSelfRect() {
			let e = this._getPoints(), t = e[0].x, n = e[0].y, r = e[0].x, i = e[0].y;
			return e.forEach((e) => {
	...
	e.RegularPolygon = a, a.prototype.className = "RegularPolygon", a.prototype._centroid = !0, a.prototype._attrsAffectingSize = ["radius"], (0, i._registerNode)(a), t.Factory.addGetterSetter(a, "radius", 0, (0, r.getNumberValidator)()), t.Factory.addGetterSetter(a, "sides", 0, (0, r.getNumberValidator)());
})), U = /* @__PURE__ */ m(((e) => {
	Object.defineProperty(e, "__esModule", { value: !0 }), e.Ring = void 0;
	var t = w(), n = k(), r = C(), i = v(), a = Math.PI * 2, o = class extends n.Shape {
		_sceneFunc(e) {
			e.beginPath(), e.arc(0, 0, this.innerRadius(), 0, a, !1), e.moveTo(this.outerRadius(), 0), e.arc(0, 0, this.outerRadius(), a, 0, !0), e.closePath(), e.fillStrokeShape(this);
		}
		getWidth() {
			return this.outerRadius() * 2;
		}
	...
	e.Ring = o, o.prototype.className = "Ring", o.prototype._centroid = !0, o.prototype._attrsAffectingSize = ["innerRadius", "outerRadius"], (0, i._registerNode)(o), t.Factory.addGetterSetter(o, "innerRadius", 0, (0, r.getNumberValidator)()), t.Factory.addGetterSetter(o, "outerRadius", 0, (0, r.getNumberValidator)());
})), ie = /* @__PURE__ */ m(((e) => {
	Object.defineProperty(e, "__esModule", { value: !0 }), e.Sprite = void 0;
	var t = w(), n = k(), r = N(), i = C(), a = v(), o = class extends n.Shape {
		constructor(e) {
			super(e), this._updated = !0, this.anim = new r.Animation(() => {
				let e = this._updated;
	...
	e.Sprite = o, o.prototype.className = "Sprite", (0, a._registerNode)(o), t.Factory.addGetterSetter(o, "animation"), t.Factory.addGetterSetter(o, "animations"), t.Factory.addGetterSetter(o, "frameOffsets"), t.Factory.addGetterSetter(o, "image"), t.Factory.addGetterSetter(o, "frameIndex", 0, (0, i.getNumberValidator)()), t.Factory.addGetterSetter(o, "frameRate", 17, (0, i.getNumberValidator)()), t.Factory.backCompat(o, {
		index: "frameIndex",
	...
	Object.defineProperty(e, "__esModule", { value: !0 }), e.Text = void 0, e.stringToArray = s;
	var t = y(), n = w(), r = k(), i = v(), a = C(), o = v();
	function s(e) {
	...
	var c = "auto", l = "center", u = "inherit", d = "justify", f = "Change.konva", p = "2d", m = "-", h = "left", g = "text", _ = "Text", b = "top", x = "bottom", S = "middle", T = "normal", E = "px ", D = " ", O = "right", A = "rtl", j = "word", M = "char", N = "none", P = "…", F = [
		"direction",
		"fontFamily",
		"fontSize",
		"fontStyle",
	...
	var R;
	function z() {
		return R || (R = t.Util.createCanvasElement().getContext(p), R);
	}
	function ee(e) {
	...
	var B = class extends r.Shape {
		constructor(e) {
			super(ne(e)), this._partialTextX = 0, this._partialTextY = 0;
			for (let e = 0; e < I; e++) this.on(F[e] + f, this._setTextData);
			this._setTextData();
		}
		_sceneFunc(e) {
			let t = this.textArr, n = t.length;
			if (!this.text()) return;
			let r = this.padding(), a = this.fontSize(), o = this.lineHeight() * a, c = this.verticalAlign(), f = this.direction(), p = 0, m = this.align(), g = this.getWidth(), _ = this.letterSpacing(), v = this.fill(), y = this.textDecoration(), b = y.indexOf("underline") !== -1, C = y.indexOf("line-through") !== -1, w;
			f = f === u ? e.direction : f;
			let T = o / 2, E = S;
			if (i.Konva._fixTextRendering) {
				let e = this.measureSize("M");
				E = "alphabetic", T = (e.fontBoundingBoxAscent - e.fontBoundingBoxDescent) / 2 + o / 2;
			}
			for (f === A && e.setAttr("direction", f), e.setAttr("font", this._getContextFont()), e.setAttr("textBaseline", E), e.setAttr("textAlign", h), c === S ? p = (this.getHeight() - n * o - r * 2) / 2 : c === x && (p = this.getHeight() - n * o - r * 2), e.translate(r, p + r), w = 0; w < n; w++) {
				let c = 0, u = t[w], p = u.text, h = u.width, y = u.lastInParagraph;
				if (e.save(), m === O ? c += g - h - r * 2 : m === l && (c += (g - h - r * 2) / 2), b) {
					e.save(), e.beginPath();
					let t = i.Konva._fixTextRendering ? Math.round(a / 4) : Math.round(a / 2), n = c, o = T + 0 + t;
					e.moveTo(n, o);
					let s = m === d && !y ? g - r * 2 : h;
					e.lineTo(n + Math.round(s), o), e.lineWidth = a / 15, e.strokeStyle = this._getLinearGradient() || v, e.stroke(), e.restore();
				}
				if (C) {
					e.save(), e.beginPath();
					let t = i.Konva._fixTextRendering ? -Math.round(a / 4) : 0;
					e.moveTo(c, T + 0 + t);
					let n = m === d && !y ? g - r * 2 : h;
					e.lineTo(c + Math.round(n), T + 0 + t), e.lineWidth = a / 15, e.strokeStyle = this._getLinearGradient() || v, e.stroke(), e.restore();
				}
				if (f !== A && (_ !== 0 || m === d)) {
					let t = p.split(" ").length - 1, n = s(p);
					for (let i = 0; i < n.length; i++) {
						let a = n[i];
						a === " " && !y && m === d && (c += (g - r * 2 - h) / t), this._partialTextX = c, this._partialTextY = T + 0, this._partialText = a, e.fillStrokeShape(this), c += this.measureSize(a).width + _;
					}
				} else _ !== 0 && e.setAttr("letterSpacing", `${_}px`), this._partialTextX = c, this._partialTextY = T + 0, this._partialText = p, e.fillStrokeShape(this);
				e.restore(), n > 1 && (T += o);
			}
		}
		_hitFunc(e) {
		...
		setText(e) {
			let n = t.Util._isString(e) ? e : e == null ? "" : e + "";
			return this._setAttr(g, n), this;
		}
		getWidth() {
		...
		_getContextFont() {
			return this.fontStyle() + D + this.fontVariant() + D + (this.fontSize() + E) + L(this.fontFamily());
		}
		_addTextLine(e) {
		...
		_getTextWidth(e) {
			...
			return z().measureText(e).width + t * n;
		}
		_setTextData() {
			let e = this.text().split("\n"), t = +this.fontSize(), n = 0, r = this.lineHeight() * t, i = this.attrs.width, a = this.attrs.height, o = i !== c && i !== void 0, l = a !== c && a !== void 0, u = this.padding(), d = i - u * 2, f = a - u * 2, p = 0, h = this.wrap(), g = h !== M && h !== N, _ = this.ellipsis();
			this.textArr = [], z().font = this._getContextFont();
			let v = _ ? this._getTextWidth(P) : 0;
			for (let t = 0, i = e.length; t < i; ++t) {
				let a = e[t], c = this._getTextWidth(a);
				if (o && c > d) for (; a.length > 0;) {
					let e = 0, t = s(a).length, i = "", o = 0;
					for (; e < t;) {
						let n = e + t >>> 1, c = s(a).slice(0, n + 1).join(""), u = this._getTextWidth(c);
						(_ && l && p + r > f ? u + v : u) <= d ? (e = n + 1, i = c, o = u) : t = n;
					}
					if (i) {
						if (g) {
							let t = s(a), n = s(i), r = t[n.length], c = r === D || r === m, l;
							if (c && o <= d) l = n.length;
							else {
								let e = n.lastIndexOf(D), t = n.lastIndexOf(m);
								l = Math.max(e, t) + 1;
							}
							l > 0 && (e = l, i = t.slice(0, e).join(""), o = this._getTextWidth(i));
						}
						if (i = i.trimRight(), this._addTextLine(i), n = Math.max(n, o), p += r, this._shouldHandleEllipsis(p)) {
							this._tryToAddEllipsisToLastLine();
							break;
						}
						if (a = s(a).slice(e).join("").trimLeft(), a.length > 0 && (c = this._getTextWidth(a), c <= d)) {
							this._addTextLine(a), p += r, n = Math.max(n, c);
							break;
						}
					} else break;
				}
				else this._addTextLine(a), p += r, n = Math.max(n, c), this._shouldHandleEllipsis(p) && t < i - 1 && this._tryToAddEllipsisToLastLine();
				if (this.textArr[this.textArr.length - 1] && (this.textArr[this.textArr.length - 1].lastInParagraph = !0), l && p + r > f) break;
			}
			this.textHeight = t, this.textWidth = n;
		}
		_shouldHandleEllipsis(e) {
			let t = +this.fontSize(), n = this.lineHeight() * t, r = this.attrs.height, i = r !== c && r !== void 0, a = r - this.padding() * 2;
			return this.wrap() === N || i && e + n > a;
		}
		_tryToAddEllipsisToLastLine() {
			let e = this.attrs.width, t = e !== c && e !== void 0, n = e - this.padding() * 2, r = this.ellipsis(), i = this.textArr[this.textArr.length - 1];
			!i || !r || (t && (this._getTextWidth(i.text + P) < n || (i.text = i.text.slice(0, i.text.length - 3))), this.textArr.splice(this.textArr.length - 1, 1), this._addTextLine(i.text + P));
		}
	...
	e.Text = B, B.prototype._fillFunc = ee, B.prototype._strokeFunc = te, B.prototype.className = _, B.prototype._attrsAffectingSize = [
		"text",
		"fontSize",
		"padding",
		"wrap",
		"lineHeight",
		"letterSpacing"
	], (0, o._registerNode)(B), n.Factory.overWriteSetter(B, "width", (0, a.getNumberOrAutoValidator)()), n.Factory.overWriteSetter(B, "height", (0, a.getNumberOrAutoValidator)()), n.Factory.addGetterSetter(B, "direction", u), n.Factory.addGetterSetter(B, "fontFamily", "Arial"), n.Factory.addGetterSetter(B, "fontSize", 12, (0, a.getNumberValidator)()), n.Factory.addGetterSetter(B, "fontStyle", T), n.Factory.addGetterSetter(B, "fontVariant", T), n.Factory.addGetterSetter(B, "padding", 0, (0, a.getNumberValidator)()), n.Factory.addGetterSetter(B, "align", h), n.Factory.addGetterSetter(B, "verticalAlign", b), n.Factory.addGetterSetter(B, "lineHeight", 1, (0, a.getNumberValidator)()), n.Factory.addGetterSetter(B, "wrap", j), n.Factory.addGetterSetter(B, "ellipsis", !1, (0, a.getBooleanValidator)()), n.Factory.addGetterSetter(B, "letterSpacing", 0, (0, a.getNumberValidator)()), n.Factory.addGetterSetter(B, "text", "", (0, a.getStringValidator)()), n.Factory.addGetterSetter(B, "textDecoration", "");
})), oe = /* @__PURE__ */ m(((e) => {
	Object.defineProperty(e, "__esModule", { value: !0 }), e.TextPath = void 0;
	var t = y(), n = w(), r = k(), i = z(), a = ae(), o = C(), s = v(), c = "", l = "normal";
	function u(e) {
	...
	var f = class extends r.Shape {
		constructor(e) {
			super(e), this.dummyCanvas = t.Util.createCanvasElement(), this.dataArray = [], this._readDataAttribute(), this.on("dataChange.konva", function() {
		...
		_getPointAtLength(e) {
			...
			let t = this.pathLength;
			return e - 1 > t ? null : i.Path.getPointAtLengthOfDataArray(e, this.dataArray);
		}
		_readDataAttribute() {
		...
		_sceneFunc(e) {
			...
			let t = this.textDecoration(), n = this.fill(), r = this.fontSize(), i = this.glyphInfo;
			t === "underline" && e.beginPath();
			for (let n = 0; n < i.length; n++) {
				e.save();
				let a = i[n].p0;
				e.translate(a.x, a.y), e.rotate(i[n].rotation), this.partialText = i[n].text, e.fillStrokeShape(this), t === "underline" && (n === 0 && e.moveTo(0, r / 2 + 1), e.lineTo(r, r / 2 + 1)), e.restore();
			}
			t === "underline" && (e.strokeStyle = n, e.lineWidth = r / 20, e.stroke()), e.restore();
		}
		_hitFunc(e) {
			e.beginPath();
		...
		_setTextData() {
			let { width: e, height: t } = this._getTextSize(this.attrs.text);
			if (this.textWidth = e, this.textHeight = t, this.glyphInfo = [], !this.attrs.data) return null;
			let n = this.letterSpacing(), r = this.align(), o = this.kerningFunc(), s = Math.max(this.textWidth + ((this.attrs.text || "").length - 1) * n, 0), c = 0;
			r === "center" && (c = Math.max(0, this.pathLength / 2 - s / 2)), r === "right" && (c = Math.max(0, this.pathLength - s));
			let l = (0, a.stringToArray)(this.text()), u = c;
			for (let e = 0; e < l.length; e++) {
				let t = this._getPointAtLength(u);
				if (!t) return;
				let a = this._getTextSize(l[e]).width + n;
				if (l[e] === " " && r === "justify") {
					let e = this.text().split(" ").length - 1;
					a += (this.pathLength - s) / e;
				}
				let c = this._getPointAtLength(u + a);
				if (!c) return;
				let d = i.Path.getLineLength(t.x, t.y, c.x, c.y), f = 0;
				if (o) try {
					f = o(l[e - 1], l[e]) * this.fontSize();
				} catch {
					f = 0;
				}
				t.x += f, c.x += f, this.textWidth += f;
				let p = i.Path.getPointOnLine(f + d / 2, t.x, t.y, c.x, c.y), m = Math.atan2(c.y - t.y, c.x - t.x);
				this.glyphInfo.push({
					transposeX: p.x,
					transposeY: p.y,
					text: l[e],
	...
	e.TextPath = f, f.prototype._fillFunc = u, f.prototype._strokeFunc = d, f.prototype._fillFuncHit = u, f.prototype._strokeFuncHit = d, f.prototype.className = "TextPath", f.prototype._attrsAffectingSize = [
		"text",
		"fontSize",
		"data"
	], (0, s._registerNode)(f), n.Factory.addGetterSetter(f, "data"), n.Factory.addGetterSetter(f, "fontFamily", "Arial"), n.Factory.addGetterSetter(f, "fontSize", 12, (0, o.getNumberValidator)()), n.Factory.addGetterSetter(f, "fontStyle", l), n.Factory.addGetterSetter(f, "align", "left"), n.Factory.addGetterSetter(f, "letterSpacing", 0, (0, o.getNumberValidator)()), n.Factory.addGetterSetter(f, "textBaseline", "middle"), n.Factory.addGetterSetter(f, "fontVariant", l), n.Factory.addGetterSetter(f, "text", c), n.Factory.addGetterSetter(f, "textDecoration", ""), n.Factory.addGetterSetter(f, "kerningFunc", void 0);
})), se = /* @__PURE__ */ m(((e) => {
	Object.defineProperty(e, "__esModule", { value: !0 }), e.Transformer = void 0;
	var t = y(), n = w(), r = T(), i = k(), a = re(), o = M(), s = v(), c = C(), l = v(), u = "tr-konva", d = [
		"resizeEnabledChange",
	...
	function D(e, t, n) {
		...
		for (let i = 0; i < e.length; i++) {
			let a = s.Konva.getAngle(e[i]), o = Math.abs(a - t) % (Math.PI * 2);
			Math.min(o, Math.PI * 2 - o) < n && (r = a);
		}
		return r;
	}
	var O = 0, A = class extends o.Group {
		constructor(e) {
			super(e), this._movingAnchorName = null, this._transforming = !1, this._createElements(), this._handleMouseMove = this._handleMouseMove.bind(this), this._handleMouseUp = this._handleMouseUp.bind(this), this.update = this.update.bind(this), this.on(d, this.update), this.getNode() && this.update();
		}
		attachTo(e) {
		...
		setNodes(e = []) {
			return this._nodes && this._nodes.length && this.detach(), this._nodes = e = e.filter((e) => e.isAncestorOf(this) ? (t.Util.error("Konva.Transformer cannot be an a child of the node you are trying to attach"), !1) : !0), e.length === 1 && this.useSingleNodeRotation() ? this.rotation(e[0].getAbsoluteRotation()) : this.rotation(0), this._nodes.forEach((e) => {
				let t = () => {
				...
				e.on(p.map((e) => e + `.${this._getEventNamespace()}`).join(" "), t), e.on(`absoluteTransformChange.${this._getEventNamespace()}`, t), this._proxyDrag(e);
		...
		__getNodeShape(e, t = this.rotation(), n) {
			...
			}), i = e.getAbsoluteScale(n), a = e.getAbsolutePosition(n), o = r.x * i.x - e.offsetX() * i.x, c = r.y * i.y - e.offsetY() * i.y, l = (s.Konva.getAngle(e.getAbsoluteRotation()) + Math.PI * 2) % (Math.PI * 2);
			return S({
				x: a.x + o * Math.cos(l) + c * Math.sin(-l),
				y: a.y + c * Math.cos(l) + o * Math.sin(l),
				width: r.width * i.x,
				height: r.height * i.y,
				rotation: l
			}, -s.Konva.getAngle(t), {
		...
		__getNodeRect() {
			...
			let n = new t.Transform();
			n.rotate(-s.Konva.getAngle(this.rotation()));
		...
		_createAnchor(e) {
			...
			t.on("mousedown touchstart", function(e) {
				...
				let n = g(e, s.Konva.getAngle(this.rotation()), this.rotateAnchorCursor());
		...
		_createBack() {
			let e = new i.Shape({
				...
				draggable: !0,
				sceneFunc(e, n) {
					let r = n.getParent(), i = r.padding();
					e.beginPath(), e.rect(-i, -i, n.width() + i * 2, n.height() + i * 2), e.moveTo(n.width() / 2, -i), r.rotateEnabled() && r.rotateLineVisible() && e.lineTo(n.width() / 2, -r.rotateAnchorOffset() * t.Util._sign(n.height()) - i), e.fillStrokeShape(n);
				},
				hitFunc: (e, t) => {
		...
		_handleMouseMove(e) {
			...
			let o = a.getPointerPosition(), c = {
				x: o.x - this._anchorDragOffset.x,
				y: o.y - this._anchorDragOffset.y
			}, l = i.getAbsolutePosition();
			this.anchorDragBoundFunc() && (c = this.anchorDragBoundFunc()(l, c, e)), i.setAbsolutePosition(c);
			let u = i.getAbsolutePosition();
			if (l.x === u.x && l.y === u.y) return;
			if (this._movingAnchorName === "rotater") {
				let r = this._getNodeRect();
				t = i.x() - r.width / 2, n = -i.y() + r.height / 2;
				let a = Math.atan2(-n, t) + Math.PI / 2;
				r.height < 0 && (a -= Math.PI);
				let o = s.Konva.getAngle(this.rotation()) + a, c = s.Konva.getAngle(this.rotationSnapTolerance()), l = E(r, D(this.rotationSnaps(), o, c) - r.rotation);
				this._fitNodesInto(l, e);
				return;
			}
			let d = this.shiftBehavior(), f;
			f = d === "inverted" ? this.keepRatio() && !e.shiftKey : d === "none" ? this.keepRatio() : this.keepRatio() || e.shiftKey;
			...
			let h = this.findOne(".top-left").getAbsolutePosition();
			t = h.x, n = h.y;
			let g = this.findOne(".bottom-right").x() - this.findOne(".top-left").x(), _ = this.findOne(".bottom-right").y() - this.findOne(".top-left").y();
			this._fitNodesInto({
				x: t,
				y: n,
				width: g,
		...
		_fitNodesInto(e, n) {
			let r = this._getNodeRect();
			if (t.Util._inRange(e.width, -this.padding() * 2 - 1, 1)) {
				this.update();
				return;
			}
			if (t.Util._inRange(e.height, -this.padding() * 2 - 1, 1)) {
				this.update();
				return;
			}
			let i = new t.Transform();
			if (i.rotate(s.Konva.getAngle(this.rotation())), this._movingAnchorName && e.width < 0 && this._movingAnchorName.indexOf("left") >= 0) {
				let t = i.point({
					x: -this.padding() * 2,
					y: 0
				});
			...
			if (this.boundBoxFunc()) {
				let n = this.boundBoxFunc()(r, e);
				n ? e = n : t.Util.warn("boundBoxFunc returned falsy. You should return new bound rect from it!");
			}
			let a = 1e7, o = new t.Transform();
			o.translate(r.x, r.y), o.rotate(r.rotation), o.scale(r.width / a, r.height / a);
			let c = new t.Transform(), l = e.width / a, u = e.height / a;
			this.flipEnabled() === !1 ? (c.translate(e.x, e.y), c.rotate(e.rotation), c.translate(e.width < 0 ? e.width : 0, e.height < 0 ? e.height : 0), c.scale(Math.abs(l), Math.abs(u))) : (c.translate(e.x, e.y), c.rotate(e.rotation), c.scale(l, u));
			let d = c.multiply(o.invert());
			this._nodes.forEach((e) => {
				var n;
				let r = e.getParent().getAbsoluteTransform(), i = e.getTransform().copy();
				i.translate(e.offsetX(), e.offsetY());
				let a = new t.Transform();
				a.multiply(r.copy().invert()).multiply(d).multiply(r).multiply(i);
				let o = a.decompose();
				e.setAttrs(o), (n = e.getLayer()) == null || n.batchDraw();
			}), this.rotation(t.Util._getRotation(e.rotation)), this._nodes.forEach((e) => {
		...
		update() {
			...
			let r = n.width, i = n.height, a = this.enabledAnchors(), o = this.resizeEnabled(), s = this.padding(), c = this.anchorSize(), l = this.find("._anchor");
			l.forEach((e) => {
				e.setAttrs({
					width: c,
					height: c,
				...
				x: r / 2,
				y: -this.rotateAnchorOffset() * t.Util._sign(i) - s,
		...
		toObject() {
			return r.Node.prototype.toObject.call(this);
		}
		clone(e) {
			return r.Node.prototype.clone.call(this, e);
		}
		getClientRect() {
	...
	e.Transformer = A, A.isTransforming = () => O > 0;
	function j(e) {
		return e instanceof Array || t.Util.warn("enabledAnchors value should be an array"), e instanceof Array && e.forEach(function(e) {
			_.indexOf(e) === -1 && t.Util.warn("Unknown anchor name: " + e + ". Available names are: " + _.join(", "));
		}), e || [];
	}
	A.prototype.className = "Transformer", (0, l._registerNode)(A), n.Factory.addGetterSetter(A, "enabledAnchors", _, j), n.Factory.addGetterSetter(A, "flipEnabled", !0, (0, c.getBooleanValidator)()), n.Factory.addGetterSetter(A, "resizeEnabled", !0), n.Factory.addGetterSetter(A, "anchorSize", 10, (0, c.getNumberValidator)()), n.Factory.addGetterSetter(A, "rotateEnabled", !0), n.Factory.addGetterSetter(A, "rotateLineVisible", !0), n.Factory.addGetterSetter(A, "rotationSnaps", []), n.Factory.addGetterSetter(A, "rotateAnchorOffset", 50, (0, c.getNumberValidator)()), n.Factory.addGetterSetter(A, "rotateAnchorCursor", "crosshair"), n.Factory.addGetterSetter(A, "rotationSnapTolerance", 5, (0, c.getNumberValidator)()), n.Factory.addGetterSetter(A, "borderEnabled", !0), n.Factory.addGetterSetter(A, "anchorStroke", "rgb(0, 161, 255)"), n.Factory.addGetterSetter(A, "anchorStrokeWidth", 1, (0, c.getNumberValidator)()), n.Factory.addGetterSetter(A, "anchorFill", "white"), n.Factory.addGetterSetter(A, "anchorCornerRadius", 0, (0, c.getNumberValidator)()), n.Factory.addGetterSetter(A, "borderStroke", "rgb(0, 161, 255)"), n.Factory.addGetterSetter(A, "borderStrokeWidth", 1, (0, c.getNumberValidator)()), n.Factory.addGetterSetter(A, "borderDash"), n.Factory.addGetterSetter(A, "keepRatio", !0), n.Factory.addGetterSetter(A, "shiftBehavior", "default"), n.Factory.addGetterSetter(A, "centeredScaling", !1), n.Factory.addGetterSetter(A, "ignoreStroke", !1), n.Factory.addGetterSetter(A, "padding", 0, (0, c.getNumberValidator)()), n.Factory.addGetterSetter(A, "nodes"), n.Factory.addGetterSetter(A, "node"), n.Factory.addGetterSetter(A, "boundBoxFunc"), n.Factory.addGetterSetter(A, "anchorDragBoundFunc"), n.Factory.addGetterSetter(A, "anchorStyleFunc"), n.Factory.addGetterSetter(A, "shouldOverdrawWholeArea", !1), n.Factory.addGetterSetter(A, "useSingleNodeRotation", !0), n.Factory.backCompat(A, {
	...
	var t = w(), n = k(), r = v(), i = C(), a = v(), o = class extends n.Shape {
		_sceneFunc(e) {
			e.beginPath(), e.arc(0, 0, this.radius(), 0, r.Konva.getAngle(this.angle()), this.clockwise()), e.lineTo(0, 0), e.closePath(), e.fillStrokeShape(this);
		}
		getWidth() {
			return this.radius() * 2;
		}
	...
	e.Wedge = o, o.prototype.className = "Wedge", o.prototype._centroid = !0, o.prototype._attrsAffectingSize = ["radius"], (0, a._registerNode)(o), t.Factory.addGetterSetter(o, "radius", 0, (0, i.getNumberValidator)()), t.Factory.addGetterSetter(o, "angle", 0, (0, i.getNumberValidator)()), t.Factory.addGetterSetter(o, "clockwise", !1), t.Factory.backCompat(o, {
		angleDeg: "angle",
	...
	function s(e, t) {
		let n = e.data, r = e.width, s = e.height, c, l, u, d, f, p, m, h, g, _, v, y, b, x, S, C, w, T, E, D, O = t + t + 1, k = r - 1, A = s - 1, j = t + 1, M = j * (j + 1) / 2, N = new i(), P = a[t], F = o[t], I = null, L = N, R = null, z = null;
		for (let e = 1; e < O; e++) L = L.next = new i(), e === j && (I = L);
		L.next = N, u = l = 0;
		for (let e = 0; e < s; e++) {
			y = b = x = S = d = f = p = m = 0, h = j * (C = n[l]), g = j * (w = n[l + 1]), _ = j * (T = n[l + 2]), v = j * (E = n[l + 3]), d += M * C, f += M * w, p += M * T, m += M * E, L = N;
			for (let e = 0; e < j; e++) L.r = C, L.g = w, L.b = T, L.a = E, L = L.next;
			for (let e = 1; e < j; e++) c = l + ((k < e ? k : e) << 2), d += (L.r = C = n[c]) * (D = j - e), f += (L.g = w = n[c + 1]) * D, p += (L.b = T = n[c + 2]) * D, m += (L.a = E = n[c + 3]) * D, y += C, b += w, x += T, S += E, L = L.next;
			R = N, z = I;
			for (let e = 0; e < r; e++) n[l + 3] = E = m * P >> F, E === 0 ? n[l] = n[l + 1] = n[l + 2] = 0 : (E = 255 / E, n[l] = (d * P >> F) * E, n[l + 1] = (f * P >> F) * E, n[l + 2] = (p * P >> F) * E), d -= h, f -= g, p -= _, m -= v, h -= R.r, g -= R.g, _ -= R.b, v -= R.a, c = u + ((c = e + t + 1) < k ? c : k) << 2, y += R.r = n[c], b += R.g = n[c + 1], x += R.b = n[c + 2], S += R.a = n[c + 3], d += y, f += b, p += x, m += S, R = R.next, h += C = z.r, g += w = z.g, _ += T = z.b, v += E = z.a, y -= C, b -= w, x -= T, S -= E, z = z.next, l += 4;
			u += r;
		}
		for (let e = 0; e < r; e++) {
			b = x = S = y = f = p = m = d = 0, l = e << 2, h = j * (C = n[l]), g = j * (w = n[l + 1]), _ = j * (T = n[l + 2]), v = j * (E = n[l + 3]), d += M * C, f += M * w, p += M * T, m += M * E, L = N;
			for (let e = 0; e < j; e++) L.r = C, L.g = w, L.b = T, L.a = E, L = L.next;
			let i = r;
			for (let a = 1; a <= t; a++) l = i + e << 2, d += (L.r = C = n[l]) * (D = j - a), f += (L.g = w = n[l + 1]) * D, p += (L.b = T = n[l + 2]) * D, m += (L.a = E = n[l + 3]) * D, y += C, b += w, x += T, S += E, L = L.next, a < A && (i += r);
			l = e, R = N, z = I;
			for (let t = 0; t < s; t++) c = l << 2, n[c + 3] = E = m * P >> F, E > 0 ? (E = 255 / E, n[c] = (d * P >> F) * E, n[c + 1] = (f * P >> F) * E, n[c + 2] = (p * P >> F) * E) : n[c] = n[c + 1] = n[c + 2] = 0, d -= h, f -= g, p -= _, m -= v, h -= R.r, g -= R.g, _ -= R.b, v -= R.a, c = e + ((c = t + j) < A ? c : A) * r << 2, d += y += R.r = n[c], f += b += R.g = n[c + 1], p += x += R.b = n[c + 2], m += S += R.a = n[c + 3], R = R.next, h += C = z.r, g += w = z.g, _ += T = z.b, v += E = z.a, y -= C, b -= w, x -= T, S -= E, z = z.next, l += r;
		}
	}
	e.Blur = function(e) {
		let t = Math.round(this.blurRadius());
		t > 0 && s(e, t);
	}, t.Factory.addGetterSetter(n.Node, "blurRadius", 0, (0, r.getNumberValidator)(), t.Factory.afterSetFilter);
})), ce = /* @__PURE__ */ m(((e) => {
	Object.defineProperty(e, "__esModule", { value: !0 }), e.Brighten = void 0;
	var t = w(), n = T(), r = C();
	e.Brighten = function(e) {
		let t = this.brightness() * 255, n = e.data, r = n.length;
		for (let e = 0; e < r; e += 4) n[e] += t, n[e + 1] += t, n[e + 2] += t;
	}, t.Factory.addGetterSetter(n.Node, "brightness", 0, (0, r.getNumberValidator)(), t.Factory.afterSetFilter);
})), le = /* @__PURE__ */ m(((e) => {
	Object.defineProperty(e, "__esModule", { value: !0 }), e.Contrast = void 0;
	var t = w(), n = T(), r = C();
	e.Contrast = function(e) {
	...
	}, t.Factory.addGetterSetter(n.Node, "contrast", 0, (0, r.getNumberValidator)(), t.Factory.afterSetFilter);
})), q = /* @__PURE__ */ m(((e) => {
	Object.defineProperty(e, "__esModule", { value: !0 }), e.Emboss = void 0;
	var t = w(), n = T(), r = y(), i = C();
	e.Emboss = function(e) {
		let t = this.embossStrength() * 10, n = this.embossWhiteLevel() * 255, i = this.embossDirection(), a = this.embossBlend(), o = e.data, s = e.width, c = e.height, l = s * 4, u = 0, d = 0, f = c;
		switch (i) {
			case "top-left":
				u = -1, d = -1;
				break;
			...
			case "left":
				u = 0, d = -1;
				break;
			default: r.Util.error("Unknown emboss direction: " + i);
		}
		do {
			let e = (f - 1) * l, r = u;
			f + r < 1 && (r = 0), f + r > c && (r = 0);
			...
			do {
				let r = e + (p - 1) * 4, c = d;
				p + c < 1 && (c = 0), p + c > s && (c = 0);
				let l = i + (p - 1 + c) * 4, u = o[r] - o[l], f = o[r + 1] - o[l + 1], m = o[r + 2] - o[l + 2], h = u, g = h > 0 ? h : -h, _ = f > 0 ? f : -f, v = m > 0 ? m : -m;
				if (_ > g && (h = f), v > g && (h = m), h *= t, a) {
					let e = o[r] + h, t = o[r + 1] + h, n = o[r + 2] + h;
					o[r] = e > 255 ? 255 : e < 0 ? 0 : e, o[r + 1] = t > 255 ? 255 : t < 0 ? 0 : t, o[r + 2] = n > 255 ? 255 : n < 0 ? 0 : n;
				} else {
					let e = n - h;
					e < 0 ? e = 0 : e > 255 && (e = 255), o[r] = o[r + 1] = o[r + 2] = e;
				}
			} while (--p);
		} while (--f);
	}, t.Factory.addGetterSetter(n.Node, "embossStrength", .5, (0, i.getNumberValidator)(), t.Factory.afterSetFilter), t.Factory.addGetterSetter(n.Node, "embossWhiteLevel", .5, (0, i.getNumberValidator)(), t.Factory.afterSetFilter), t.Factory.addGetterSetter(n.Node, "embossDirection", "top-left", void 0, t.Factory.afterSetFilter), t.Factory.addGetterSetter(n.Node, "embossBlend", !1, void 0, t.Factory.afterSetFilter);
})), J = /* @__PURE__ */ m(((e) => {
	...
	e.Enhance = function(e) {
		let t = e.data, n = t.length, r = t[0], a = r, o, s = t[1], c = s, l, u = t[2], d = u, f, p = this.enhance();
		if (p === 0) return;
		for (let e = 0; e < n; e += 4) o = t[e + 0], o < r ? r = o : o > a && (a = o), l = t[e + 1], l < s ? s = l : l > c && (c = l), f = t[e + 2], f < u ? u = f : f > d && (d = f);
		a === r && (a = 255, r = 0), c === s && (c = 255, s = 0), d === u && (d = 255, u = 0);
		let m, h, g, _, v, y;
		if (p > 0) m = a + p * (255 - a), h = r - p * (r - 0), g = c + p * (255 - c), _ = s - p * (s - 0), v = d + p * (255 - d), y = u - p * (u - 0);
		else {
			let e = (a + r) * .5;
			m = a + p * (a - e), h = r + p * (r - e);
			let t = (c + s) * .5;
		...
		for (let e = 0; e < n; e += 4) t[e + 0] = i(t[e + 0], r, a, h, m), t[e + 1] = i(t[e + 1], s, c, _, g), t[e + 2] = i(t[e + 2], u, d, y, v);
	}, t.Factory.addGetterSetter(n.Node, "enhance", 0, (0, r.getNumberValidator)(), t.Factory.afterSetFilter);
})), ue = /* @__PURE__ */ m(((e) => {
	Object.defineProperty(e, "__esModule", { value: !0 }), e.Grayscale = void 0, e.Grayscale = function(e) {
	...
	var t = w(), n = T(), r = C();
	t.Factory.addGetterSetter(n.Node, "hue", 0, (0, r.getNumberValidator)(), t.Factory.afterSetFilter), t.Factory.addGetterSetter(n.Node, "saturation", 0, (0, r.getNumberValidator)(), t.Factory.afterSetFilter), t.Factory.addGetterSetter(n.Node, "luminance", 0, (0, r.getNumberValidator)(), t.Factory.afterSetFilter), e.HSL = function(e) {
	...
	}, t.Factory.addGetterSetter(n.Node, "hue", 0, (0, r.getNumberValidator)(), t.Factory.afterSetFilter), t.Factory.addGetterSetter(n.Node, "saturation", 0, (0, r.getNumberValidator)(), t.Factory.afterSetFilter), t.Factory.addGetterSetter(n.Node, "value", 0, (0, r.getNumberValidator)(), t.Factory.afterSetFilter);
})), Y = /* @__PURE__ */ m(((e) => {
	Object.defineProperty(e, "__esModule", { value: !0 }), e.Invert = void 0, e.Invert = function(e) {
		let t = e.data, n = t.length;
	...
	var t = w(), n = T(), r = y(), i = C(), a = function(e, t, n) {
	...
	e.Kaleidoscope = function(e) {
		let t = e.width, n = e.height, i, s, c, l, u, d, f, p, m, h, g = Math.round(this.kaleidoscopePower()), _ = Math.round(this.kaleidoscopeAngle()), v = Math.floor(_ % 360 * t / 360);
		if (g < 1) return;
		let y = r.Util.createCanvasElement();
		y.width = t, y.height = n;
		let b = y.getContext("2d").getImageData(0, 0, t, n);
		r.Util.releaseCanvas(y), a(e, b, {
			polarCenterX: t / 2,
			polarCenterY: n / 2
		});
		let x = t / 2 ** g;
		...
		x = Math.ceil(x);
		let S = x, C = 0, w = S, T = 1;
		for (v + x > t && (C = S, w = 0, T = -1), s = 0; s < n; s += 1) for (i = C; i !== w; i += T) c = Math.round(i + v) % t, m = (t * s + c) * 4, u = b.data[m + 0], d = b.data[m + 1], f = b.data[m + 2], p = b.data[m + 3], h = (t * s + i) * 4, b.data[h + 0] = u, b.data[h + 1] = d, b.data[h + 2] = f, b.data[h + 3] = p;
		for (s = 0; s < n; s += 1) for (S = Math.floor(x), l = 0; l < g; l += 1) {
			for (i = 0; i < S + 1; i += 1) m = (t * s + i) * 4, u = b.data[m + 0], d = b.data[m + 1], f = b.data[m + 2], p = b.data[m + 3], h = (t * s + S * 2 - i - 1) * 4, b.data[h + 0] = u, b.data[h + 1] = d, b.data[h + 2] = f, b.data[h + 3] = p;
			S *= 2;
		}
		o(b, e, { polarRotation: 0 });
	}, t.Factory.addGetterSetter(n.Node, "kaleidoscopePower", 2, (0, i.getNumberValidator)(), t.Factory.afterSetFilter), t.Factory.addGetterSetter(n.Node, "kaleidoscopeAngle", 0, (0, i.getNumberValidator)(), t.Factory.afterSetFilter);
})), me = /* @__PURE__ */ m(((e) => {
	...
	function s(e, t) {
		let n = i(e, 0, 0), r = i(e, e.width - 1, 0), s = i(e, 0, e.height - 1), c = i(e, e.width - 1, e.height - 1), l = t || 10;
		if (a(n, r) < l && a(r, c) < l && a(c, s) < l && a(s, n) < l) {
			let t = o([
				r,
				n,
				c,
			...
			for (let n = 0; n < e.width * e.height; n++) i[n] = a(t, [
				e.data[n * 4],
				e.data[n * 4 + 1],
				e.data[n * 4 + 2]
			]) < l ? 0 : 255;
	...
	e.Mask = function(e) {
		let t = s(e, this.threshold());
		return t && (t = l(t, e.width, e.height), t = u(t, e.width, e.height), t = d(t, e.width, e.height), c(e, t)), e;
	}, t.Factory.addGetterSetter(n.Node, "threshold", 0, (0, r.getNumberValidator)(), t.Factory.afterSetFilter);
})), he = /* @__PURE__ */ m(((e) => {
	...
	}, t.Factory.addGetterSetter(n.Node, "noise", .2, (0, r.getNumberValidator)(), t.Factory.afterSetFilter);
})), ge = /* @__PURE__ */ m(((e) => {
	Object.defineProperty(e, "__esModule", { value: !0 }), e.Pixelate = void 0;
	var t = w(), n = y(), r = T(), i = C();
	e.Pixelate = function(e) {
		let t = Math.ceil(this.pixelSize()), r = e.width, i = e.height, a = Math.ceil(r / t), o = Math.ceil(i / t), s = e.data;
		if (t <= 0) {
			n.Util.error("pixelSize value can not be <= 0");
			return;
		}
		for (let e = 0; e < a; e += 1) for (let n = 0; n < o; n += 1) {
			let a = 0, o = 0, c = 0, l = 0, u = e * t, d = u + t, f = n * t, p = f + t, m = 0;
			for (let e = u; e < d; e += 1) if (!(e >= r)) for (let t = f; t < p; t += 1) {
				if (t >= i) continue;
				let n = (r * t + e) * 4;
				a += s[n + 0], o += s[n + 1], c += s[n + 2], l += s[n + 3], m += 1;
			}
			a /= m, o /= m, c /= m, l /= m;
			for (let e = u; e < d; e += 1) if (!(e >= r)) for (let t = f; t < p; t += 1) {
				if (t >= i) continue;
				let n = (r * t + e) * 4;
				s[n + 0] = a, s[n + 1] = o, s[n + 2] = c, s[n + 3] = l;
			}
		}
	}, t.Factory.addGetterSetter(r.Node, "pixelSize", 8, (0, i.getNumberValidator)(), t.Factory.afterSetFilter);
})), _e = /* @__PURE__ */ m(((e) => {
	Object.defineProperty(e, "__esModule", { value: !0 }), e.Posterize = void 0;
	var t = w(), n = T(), r = C();
	e.Posterize = function(e) {
		let t = Math.round(this.levels() * 254) + 1, n = e.data, r = n.length, i = 255 / t;
		for (let e = 0; e < r; e += 1) n[e] = Math.floor(n[e] / i) * i;
	}, t.Factory.addGetterSetter(n.Node, "levels", .5, (0, r.getNumberValidator)(), t.Factory.afterSetFilter);
})), ve = /* @__PURE__ */ m(((e) => {
	...
	}, t.Factory.addGetterSetter(n.Node, "red", 0, function(e) {
		return this._filterUpToDate = !1, e > 255 ? 255 : e < 0 ? 0 : Math.round(e);
	}), t.Factory.addGetterSetter(n.Node, "green", 0, function(e) {
		return this._filterUpToDate = !1, e > 255 ? 255 : e < 0 ? 0 : Math.round(e);
	}), t.Factory.addGetterSetter(n.Node, "blue", 0, r.RGBComponent, t.Factory.afterSetFilter);
})), ye = /* @__PURE__ */ m(((e) => {
	...
	e.RGBA = function(e) {
		...
		return this._filterUpToDate = !1, e > 255 ? 255 : e < 0 ? 0 : Math.round(e);
	}), t.Factory.addGetterSetter(n.Node, "blue", 0, r.RGBComponent, t.Factory.afterSetFilter), t.Factory.addGetterSetter(n.Node, "alpha", 1, function(e) {
		return this._filterUpToDate = !1, e > 1 ? 1 : e < 0 ? 0 : e;
	});
})), be = /* @__PURE__ */ m(((e) => {
	Object.defineProperty(e, "__esModule", { value: !0 }), e.Sepia = void 0, e.Sepia = function(e) {
		let t = e.data, n = t.length;
		for (let e = 0; e < n; e += 4) {
	...
	Object.defineProperty(e, "__esModule", { value: !0 }), e.Solarize = void 0, e.Solarize = function(e) {
		let t = e.data, n = e.width, r = e.height, i = n * 4, a = r;
		do {
			let e = (a - 1) * i, r = n;
			do {
				let n = e + (r - 1) * 4, i = t[n], a = t[n + 1], o = t[n + 2];
				i > 127 && (i = 255 - i), a > 127 && (a = 255 - a), o > 127 && (o = 255 - o), t[n] = i, t[n + 1] = a, t[n + 2] = o;
			} while (--r);
		} while (--a);
	};
})), Se = /* @__PURE__ */ m(((e) => {
	...
	}, t.Factory.addGetterSetter(n.Node, "threshold", .5, (0, r.getNumberValidator)(), t.Factory.afterSetFilter);
})), Ce = /* @__PURE__ */ m(((e) => {
	Object.defineProperty(e, "__esModule", { value: !0 }), e.Konva = void 0;
	var t = F(), n = I(), r = ee(), i = te(), a = ne(), o = B(), s = V(), c = L(), l = z(), u = re(), d = H(), f = U(), p = ie(), m = W(), h = ae(), g = oe(), _ = se(), v = G(), y = K(), b = ce(), x = le(), S = q(), C = J(), w = ue(), T = de(), E = fe(), D = Y(), O = pe(), k = me(), A = he(), j = ge(), M = _e(), N = ve(), P = ye(), R = be(), Ce = xe(), X = Se();
	e.Konva = t.Konva.Util._assign(t.Konva, {
		Arc: n.Arc,
		Arrow: r.Arrow,
		Circle: i.Circle,
		Ellipse: a.Ellipse,
		...
		Filters: {
			...
			RGBA: P.RGBA,
			Sepia: R.Sepia,
			Solarize: Ce.Solarize,
			Threshold: X.Threshold
		}
	});
})), X = /* @__PURE__ */ g((/* @__PURE__ */ m(((e, t) => {
	Object.defineProperty(e, "__esModule", { value: !0 }), t.exports = Ce().Konva;
})))(), 1), we = class {
	constructor(e) {
	...
	(function(n, r) {
		...
		let a = {}, o = () => {
		...
		}, v = (e) => typeof e == "function" ? e() : e, y = (e) => e && typeof e.toPromise == "function", b = (e) => y(e) ? e.toPromise() : Promise.resolve(e), x = (e) => e && Promise.resolve(e) === e, S = () => document.body.querySelector(`.${l.container}`), C = (e) => {
			let t = S();
			return t ? t.querySelector(e) : null;
		}, w = (e) => C(`.${e}`), T = () => w(l.popup), E = () => w(l.icon), D = () => w(l["icon-content"]), O = () => w(l.title), k = () => w(l["html-container"]), A = () => w(l.image), j = () => w(l["progress-steps"]), M = () => w(l["validation-message"]), N = () => C(`.${l.actions} .${l.confirm}`), P = () => C(`.${l.actions} .${l.cancel}`), F = () => C(`.${l.actions} .${l.deny}`), I = () => w(l["input-label"]), L = () => C(`.${l.loader}`), R = () => w(l.actions), z = () => w(l.footer), ee = () => w(l["timer-progress-bar"]), te = () => w(l.close), ne = () => {
			let e = T();
			if (!e) return [];
			...
			return [...new Set(n.concat(i))].filter((e) => Y(e));
		}, B = () => U(document.body, l.shown) && !U(document.body, l["toast-shown"]) && !U(document.body, l["no-backdrop"]), V = () => {
			let e = T();
			return e ? U(e, l.toast) : !1;
		}, re = () => {
			let e = T();
			...
			if (ie(e, t), !t.customClass) return;
			...
			for (let e = 0; e < n.length; e++) {
				let r = n[e];
				if (r instanceof HTMLElement && U(r, t)) return r;
			}
		}, le = (e, t, n) => {
			...
			t ? q(e, n) : J(e);
		}, Y = (e) => !!(e && (e.offsetWidth || e.offsetHeight || e.getClientRects().length)), pe = () => !Y(N()) && !Y(F()) && !Y(P()), me = (e) => e.scrollHeight > e.clientHeight, he = (e, t) => {
			let n = e;
			for (; n && n !== t;) {
				if (me(n)) return !0;
				n = n.parentElement;
			...
			let n = ee();
			n && Y(n) && (t && (n.style.transition = "none", n.style.width = "100%"), setTimeout(() => {
			...
			e.style.width = `${n}%`;
		}, ye = () => typeof window > "u" || typeof document > "u", be = `
...
`.replace(/(^|\n)\s*/g, ""), xe = () => {
			let e = S();
			return e ? (e.remove(), K([document.documentElement, document.body], [
				l["no-backdrop"],
				l["toast-shown"],
				l["has-column"]
			]), !0) : !1;
			...
			let e = T();
			if (!e) return;
			let t = ce(e, l.input), n = ce(e, l.file), r = e.querySelector(`.${l.range} input`), i = e.querySelector(`.${l.range} output`), a = ce(e, l.select), o = e.querySelector(`.${l.checkbox} input`), s = ce(e, l.textarea);
			t && (t.oninput = Se), n && (n.onchange = Se), a && (a.onchange = Se), o && (o.onchange = Se), s && (s.oninput = Se), r && i && (r.oninput = () => {
				Se(), i.value = r.value;
			}, r.onchange = () => {
				Se(), i.value = r.value;
			});
			...
			window.getComputedStyle(e).direction === "rtl" && (G(S(), l.rtl), a.isRTL = !0);
		}, Ee = (e) => {
			let t = xe();
			if (ye()) {
			...
			n.className = l.container, t && G(n, l["no-transition"]), H(n, be), n.dataset.swal2Theme = e.theme;
			let r = X(e.target || "body");
			r.appendChild(n), e.topLayer && (n.setAttribute("popover", ""), n.showPopover()), we(e), Te(r), Ce();
		}, De = (e, t) => {
			e instanceof HTMLElement ? t.appendChild(e) : typeof e == "object" ? Oe(e, t) : e && H(t, e);
		}, Oe = (e, t) => {
			"jquery" in e ? ke(t, e) : H(t, e.toString());
		}, ke = (e, t) => {
			if (e.textContent = "", 0 in t) for (let n = 0; n in t; n++) e.appendChild(t[n].cloneNode(!0));
			else e.appendChild(t.cloneNode(!0));
		}, Ae = (e, t) => {
			let n = R(), r = L();
			!n || !r || (!t.showConfirmButton && !t.showDenyButton && !t.showCancelButton ? J(n) : q(n), W(n, t, "actions"), je(n, r, t), H(r, t.loaderHtml || ""), W(r, t, "loader"));
		};
		function je(e, t, n) {
			let r = N(), i = F(), a = P();
			!r || !i || !a || (Pe(r, "confirm", n), Pe(i, "deny", n), Pe(a, "cancel", n), Me(r, i, a, n), n.reverseButtons && (n.toast ? (e.insertBefore(a, r), e.insertBefore(i, r)) : (e.insertBefore(a, t), e.insertBefore(i, t), e.insertBefore(r, t))));
		}
		function Me(e, t, n, r) {
			if (!r.buttonsStyling) {
				K([
			...
			G([
				...
				n && e.style.setProperty(`--swal2-${t}-button-background-color`, n), Ne(e);
			});
		}
		function Ne(e) {
		...
		function Pe(e, t, n) {
			fe(e, n[`show${f(t)}Button`], "inline-block"), H(e, n[`${t}ButtonText`] || ""), e.setAttribute("aria-label", n[`${t}ButtonAriaLabel`] || ""), e.className = l[t], W(e, n, `${t}Button`);
		}
		let Fe = (e, t) => {
			let n = te();
			n && (H(n, t.closeButtonHtml || ""), W(n, t, "closeButton"), fe(n, t.showCloseButton), n.setAttribute("aria-label", t.closeButtonAriaLabel || ""));
		}, Ie = (e, t) => {
			let n = S();
			n && (Le(n, t.backdrop), Re(n, t.position), ze(n, t.grow), W(n, t, "container"));
		};
		function Le(e, t) {
			typeof t == "string" ? e.style.background = t : t || G([document.documentElement, document.body], l["no-backdrop"]);
		}
		function Re(e, t) {
			t && (t in l ? G(e, l[t]) : (p("The \"position\" parameter is not valid, defaulting to \"center\""), G(e, l.center)));
		}
		function ze(e, t) {
			t && G(e, l[`grow-${t}`]);
		}
		...
		let Be = [
			...
			Be.forEach((e) => {
				let r = ce(n, l[e]);
				r && (We(e, t.inputAttributes), r.className = l[e], i && J(r));
			}), t.input && (i && He(t), Ge(t));
		}, He = (e) => {
			if (!e.input) return;
			if (!Q[e.input]) {
				m(`Unexpected type of input! Expected ${Object.keys(Q).join(" | ")}, got "${e.input}"`);
				return;
			}
			let t = Je(e.input);
			if (!t) return;
			let n = Q[e.input](t, e);
			q(t), e.inputAutoFocus && setTimeout(() => {
				oe(n);
			});
		}, Ue = (e) => {
			...
			if (n.inputLabel) {
				...
				r.setAttribute("for", e.id), r.className = i, typeof n.customClass == "object" && G(r, n.customClass.inputLabel), r.innerText = n.inputLabel, t.insertAdjacentElement("beforebegin", r);
			}
		}, Je = (e) => {
			let t = T();
			if (t) return ce(t, l[e] || l.input);
		}, Ye = (e, t) => {
			["string", "number"].includes(typeof t) ? e.value = `${t}` : x(t) || p(`Unexpected type of inputValue! Expected "string", "number" or "Promise", got "${typeof t}"`);
		}, Q = {};
		Q.text = Q.email = Q.password = Q.number = Q.tel = Q.url = Q.search = Q.date = Q["datetime-local"] = Q.time = Q.week = Q.month = (e, t) => {
			let n = e;
			return Ye(n, t.inputValue), qe(n, n, t), Ke(n, t), n.type = t.input, n;
		}, Q.file = (e, t) => {
			let n = e;
			return qe(n, n, t), Ke(n, t), n;
		}, Q.range = (e, t) => {
			let n = e, r = n.querySelector("input"), i = n.querySelector("output");
			return r && (Ye(r, t.inputValue), r.type = t.input, qe(r, e, t)), i && Ye(i, t.inputValue), e;
		}, Q.select = (e, t) => {
			let n = e;
			if (n.textContent = "", t.inputPlaceholder) {
				let e = document.createElement("option");
				H(e, t.inputPlaceholder), e.value = "", e.disabled = !0, e.selected = !0, n.appendChild(e);
			}
			return qe(n, n, t), n;
		}, Q.radio = (e) => {
			let t = e;
			...
			let n = e;
			Ye(n, t.inputValue), Ke(n, t), qe(n, n, t);
			...
			return setTimeout(() => {
				if ("MutationObserver" in window) {
					...
					new MutationObserver(() => {
						...
						let e = n.offsetWidth + r(n), a = T();
						a && (e > i ? a.style.width = `${e}px` : le(a, "width", t.width));
					}).observe(n, {
						attributes: !0,
		...
		let Xe = (e, t) => {
			let n = k();
			n && (ue(n), W(n, t, "htmlContainer"), t.html ? (De(t.html, n), q(n, "block")) : t.text ? (n.textContent = t.text, q(n, "block")) : J(n), Ve(e, t));
		}, Ze = (e, t) => {
			let n = z();
			n && (ue(n), fe(n, !!t.footer, "block"), t.footer && De(t.footer, n), W(n, t, "footer"));
		}, Qe = (e, t) => {
			let n = Z.innerParams.get(e), r = E();
			if (r) {
				if (n && t.icon === n.icon) {
					nt(r, t), $e(r, t);
					return;
				...
				if (t.icon && Object.keys(u).indexOf(t.icon) === -1) {
					m(`Unknown icon! Expected "success", "error", "warning", "info" or "question", got "${t.icon}"`), J(r);
					return;
				}
				q(r), nt(r, t), $e(r, t), G(r, t.showClass && t.showClass.icon), window.matchMedia("(prefers-color-scheme: dark)").addEventListener("change", et);
			}
		}, $e = (e, t) => {
			for (let [n, r] of Object.entries(u)) t.icon !== n && K(e, r);
			G(e, t.icon && u[t.icon]), rt(e, t), et(), W(e, t, "icon");
		}, et = () => {
			let e = T();
			if (!e) return;
...
`, nt = (e, t) => {
			...
			let n = e.innerHTML, r = "";
			t.iconHtml ? r = it(t.iconHtml) : t.icon === "success" ? (r = tt(t), n = n.replace(/ style=".*?"/g, "")) : t.icon === "error" ? r = "\n  <span class=\"swal2-x-mark\">\n    <span class=\"swal2-x-mark-line-left\"></span>\n    <span class=\"swal2-x-mark-line-right\"></span>\n  </span>\n" : t.icon && (r = it({
				question: "?",
				warning: "!",
				info: "i"
			}[t.icon])), n.trim() !== r.trim() && H(e, r);
		}, rt = (e, t) => {
			if (t.iconColor) {
				...
				]) de(e, n, "background-color", t.iconColor);
				de(e, ".swal2-success-ring", "border-color", t.iconColor);
			}
		}, it = (e) => `<div class="${l["icon-content"]}">${e}</div>`, at = (e, t) => {
			let n = A();
			if (n) {
				if (!t.imageUrl) {
					J(n);
				...
				q(n, ""), n.setAttribute("src", t.imageUrl), n.setAttribute("alt", t.imageAlt || ""), le(n, "width", t.imageWidth), le(n, "height", t.imageHeight), n.className = l.image, W(n, t, "image");
			}
		}, ot = !1, st = 0, ct = 0, lt = 0, ut = 0, dt = (e) => {
			e.addEventListener("mousedown", pt), document.body.addEventListener("mousemove", mt), e.addEventListener("mouseup", ht), e.addEventListener("touchstart", pt), document.body.addEventListener("touchmove", mt), e.addEventListener("touchend", ht);
		}, ft = (e) => {
			e.removeEventListener("mousedown", pt), document.body.removeEventListener("mousemove", mt), e.removeEventListener("mouseup", ht), e.removeEventListener("touchstart", pt), document.body.removeEventListener("touchmove", mt), e.removeEventListener("touchend", ht);
		}, pt = (e) => {
			let t = T();
			if (!t) return;
			let n = E();
			if (e.target === t || n && n.contains(e.target)) {
				ot = !0;
				let n = gt(e);
				st = n.clientX, ct = n.clientY, lt = parseInt(t.style.insetInlineStart) || 0, ut = parseInt(t.style.insetBlockStart) || 0, G(t, "swal2-dragging");
			}
		}, mt = (e) => {
			let t = T();
			if (t && ot) {
				let { clientX: n, clientY: r } = gt(e), i = n - st;
				t.style.insetInlineStart = `${lt + (a.isRTL ? -i : i)}px`, t.style.insetBlockStart = `${ut + (r - ct)}px`;
			}
		}, ht = () => {
			let e = T();
			ot = !1, K(e, "swal2-dragging");
		}, gt = (e) => {
			...
			if (!(!n || !r)) {
				if (t.toast) {
					...
					let e = L();
					e && r.insertBefore(e, E());
				} else le(r, "width", t.width);
				le(r, "padding", t.padding), t.color && (r.style.color = t.color), t.background && (r.style.background = t.background), J(M()), vt(r, t), t.draggable && !t.toast ? (G(r, l.draggable), dt(r)) : (K(r, l.draggable), ft(r));
			}
		}, vt = (e, t) => {
			let n = t.showClass || {};
			e.className = `${l.popup} ${Y(e) ? n.popup : ""}`, t.toast ? (G([document.documentElement, document.body], l["toast-shown"]), G(e, l.toast)) : G(e, l.modal), W(e, t, "popup"), typeof t.customClass == "string" && G(e, t.customClass), t.icon && G(e, l[`icon-${t.icon}`]);
		}, yt = (e, t) => {
			let n = j();
			if (!n) return;
			...
			q(n), n.textContent = "", i >= r.length && p("Invalid currentProgressStep parameter, it should be less than progressSteps.length (currentProgressStep like JS arrays starts from 0)"), r.forEach((e, a) => {
				let o = bt(e);
				if (n.appendChild(o), a === i && G(o, l["active-progress-step"]), a !== r.length - 1) {
					let e = xt(t);
					n.appendChild(e);
				}
			});
			...
			return G(t, l["progress-step"]), H(t, e), t;
		}, xt = (e) => {
			let t = document.createElement("li");
			return G(t, l["progress-step-line"]), e.progressStepsDistance && le(t, "width", e.progressStepsDistance), t;
		}, St = (e, t) => {
			let n = O();
			n && (ue(n), fe(n, !!(t.title || t.titleText), "block"), t.title && De(t.title, n), t.titleText && (n.innerText = t.titleText), W(n, t, "title"));
		}, Ct = (e, t) => {
			var n;
			_t(e, t), Ie(e, t), yt(e, t), Qe(e, t), at(e, t), St(e, t), Fe(e, t), Xe(e, t), Ae(e, t), Ze(e, t);
			let r = T();
			typeof t.didRender == "function" && r && t.didRender(r), (n = a.eventEmitter) == null || n.emit("didRender", r);
		}, wt = () => Y(T()), Tt = () => N()?.click(), Et = () => F()?.click(), Dt = () => P()?.click(), Ot = Object.freeze({
			cancel: "cancel",
			backdrop: "backdrop",
			close: "close",
			esc: "esc",
			...
			if (kt(e), !t.toast) {
				let r = (e) => Pt(t, e, n);
				e.keydownHandler = r;
				let i = t.keydownListenerCapture ? window : T();
				if (i) {
			...
			let r = ne();
			if (r.length) {
				e += t, e === -2 && (e = r.length - 1), e === r.length ? e = 0 : e === -1 && (e = r.length - 1), r[e].focus();
				return;
			}
			(n = T()) == null || n.focus();
		}, Mt = ["ArrowRight", "ArrowDown"], Nt = ["ArrowLeft", "ArrowUp"], Pt = (e, t, n) => {
			e && (t.isComposing || t.keyCode === 229 || (e.stopKeydownPropagation && t.stopPropagation(), t.key === "Enter" ? Ft(t, e) : t.key === "Tab" ? It(t) : [...Mt, ...Nt].includes(t.key) ? Lt(t.key) : t.key === "Escape" && Rt(t, e, n)));
		}, Ft = (e, t) => {
			...
			let o = Mt.includes(e) ? "nextElementSibling" : "previousElementSibling", s = document.activeElement;
			if (s) {
				for (let e = 0; e < t.children.length; e++) {
					if (s = s[o], !s) return;
					if (s instanceof HTMLButtonElement && Y(s)) break;
				}
				s instanceof HTMLButtonElement && s.focus();
			}
		}, Rt = (e, t, n) => {
			e.preventDefault(), v(t.allowEscapeKey) && n(Ot.esc);
		};
		var zt = {
		...
		let Bt = () => {
		...
		}, Ht = typeof window < "u" && !!window.GestureEvent, Ut = Ht && /iPad|iPhone|iPod/.test(navigator.userAgent) && !window.MSStream, Wt = () => {
			if (Ht && !U(document.body, l.iosfix)) {
				let e = document.body.scrollTop;
				document.body.style.top = `${e * -1}px`, G(document.body, l.iosfix), Gt();
			}
		}, Gt = () => {
			let e = S();
			if (!e) return;
			...
			let t = e.target, n = S(), r = k();
			return !n || !r || qt(e) || Jt(e) ? !1 : t === n || !me(n) && t instanceof HTMLElement && !he(t, r) && t.tagName !== "INPUT" && t.tagName !== "TEXTAREA" && !(me(r) && r.contains(t));
		}, qt = (e) => !!(e.touches && e.touches.length && e.touches[0].touchType === "stylus"), Jt = (e) => e.touches && e.touches.length > 1, Yt = () => {
			if (U(document.body, l.iosfix)) {
				let e = parseInt(document.body.style.top, 10);
				K(document.body, l.iosfix), document.body.style.top = "", document.body.scrollTop = e * -1;
			}
		}, Xt = () => {
			...
			Zt === null && (document.body.scrollHeight > window.innerHeight || e === "scroll") && (Zt = parseInt(window.getComputedStyle(document.body).getPropertyValue("padding-right")), document.body.style.paddingRight = `${Zt + Xt()}px`);
		}, $t = () => {
			Zt !== null && (document.body.style.paddingRight = `${Zt}px`, Zt = null);
		};
		function en(e, t, n, r) {
			V() ? un(e, r) : (s(n).then(() => un(e, r)), kt(a)), Ht ? (t.setAttribute("style", "display:none !important"), t.removeAttribute("class"), t.innerHTML = "") : t.remove(), B() && ($t(), Yt(), Vt()), tn();
		}
		function tn() {
		...
		function nn(e) {
			e = sn(e);
			let t = zt.swalPromiseResolve.get(this), n = rn(this);
			this.isAwaitingPromise ? e.isDismissed || (on(this), t(e)) : n && t(e);
		}
		let rn = (e) => {
			let t = T();
			if (!t) return !1;
			...
			K(t, n.showClass.popup), G(t, n.hideClass.popup);
			let r = S();
			return K(r, n.showClass.backdrop), G(r, n.hideClass.backdrop), cn(e, t, n), !0;
		};
		function an(e) {
			let t = zt.swalPromiseReject.get(this);
			on(this), t && t(e);
		}
		let on = (e) => {
			...
			var r;
			let i = S(), o = ge(t);
			typeof n.willClose == "function" && n.willClose(t), (r = a.eventEmitter) == null || r.emit("willClose", t), o && i ? ln(e, t, i, !!n.returnFocus, n.didClose) : i && en(e, i, !!n.returnFocus, n.didClose);
		}, ln = (e, t, n, r, i) => {
			a.swalCloseEventFinishedCallback = en.bind(null, e, n, r, i);
			let o = function(e) {
				if (e.target === t) {
					var n;
			...
			let t = T();
			if (t || new ci(), t = T(), !t) return;
			let n = L();
			V() ? J(E()) : fn(t, e), q(n), t.setAttribute("data-loading", "true"), t.setAttribute("aria-busy", "true"), t.focus();
		}, fn = (e, t) => {
			let n = R(), r = L();
			!n || !r || (!t && Y(N()) && (t = N()), q(n), t && (J(t), r.setAttribute("data-button-to-replace", t.className), n.insertBefore(r, t)), G([e, n], l.loading));
		}, pn = (e, t) => {
			t.input === "select" || t.input === "radio" ? vn(e, t) : [
				"text",
				"email",
				"number",
				"tel",
				"textarea"
			].some((e) => e === t.input) && (y(t.inputValue) || x(t.inputValue)) && (dn(N()), yn(e, t));
		}, mn = (e, t) => {
			let n = e.getInput();
			if (!n) return null;
			switch (t.input) {
				case "checkbox": return hn(n);
				case "radio": return gn(n);
				case "file": return _n(n);
				default: return t.inputAutoTrim ? n.value.trim() : n.value;
			}
		}, hn = (e) => e.checked ? 1 : 0, gn = (e) => e.checked ? e.value : null, _n = (e) => e.files && e.files.length ? e.getAttribute("multiple") === null ? e.files[0] : e.files : null, vn = (e, t) => {
			let n = T();
			if (!n) return;
			let r = (e) => {
				t.input === "select" ? bn(n, Sn(e), t) : t.input === "radio" && xn(n, Sn(e), t);
			};
			y(t.inputOptions) || x(t.inputOptions) ? (dn(N()), b(t.inputOptions).then((t) => {
				e.hideLoading(), r(t);
			...
			let n = e.getInput();
			n && (J(n), b(t.inputValue).then((r) => {
				n.value = t.input === "number" ? `${parseFloat(r) || 0}` : `${r}`, q(n), n.focus(), e.hideLoading();
			}).catch((t) => {
				m(`Error in inputValue promise: ${t}`), n.value = "", q(n), n.focus(), e.hideLoading();
			}));
		};
		function bn(e, t, n) {
			let r = ce(e, l.select);
			...
			let i = (e, t, r) => {
				...
				i.value = r, H(i, t), i.selected = Cn(r, n.inputValue), e.appendChild(i);
			};
			t.forEach((e) => {
				let t = e[0], n = e[1];
				if (Array.isArray(n)) {
		...
		function xn(e, t, n) {
			...
			t.forEach((e) => {
				...
				a.type = "radio", a.name = l.radio, a.value = t, Cn(t, n.inputValue) && (a.checked = !0);
				let s = document.createElement("span");
				H(s, i), s.className = l.label, o.appendChild(a), o.appendChild(s), r.appendChild(o);
			});
			let i = r.querySelectorAll("input");
			i.length && i[0].focus();
		}
		let Sn = (e) => (e instanceof Map ? Array.from(e) : Object.entries(e)).map(([e, t]) => [e, typeof t == "object" ? Sn(t) : t]), Cn = (e, t) => !!t && t != null && t.toString() === e.toString(), wn = (e) => {
			let t = Z.innerParams.get(e);
			e.disableButtons(), t.input ? Dn(e, "confirm") : Mn(e, !0);
		}, Tn = (e) => {
			let t = Z.innerParams.get(e);
			e.disableButtons(), t.returnInputValueOnDeny ? Dn(e, "deny") : kn(e, !1);
		}, En = (e, t) => {
			e.disableButtons(), t(Ot.cancel);
		}, Dn = (e, t) => {
			let n = Z.innerParams.get(e);
			if (!n.input) {
			...
			let r = e.getInput(), i = mn(e, n);
			n.inputValidator ? On(e, i, t) : r && !r.checkValidity() ? (e.enableButtons(), e.showValidationMessage(n.validationMessage || r.validationMessage)) : t === "deny" ? kn(e, i) : Mn(e, i);
		}, On = (e, t, n) => {
			let r = Z.innerParams.get(e);
			e.disableInput(), Promise.resolve().then(() => b(r.inputValidator(t, r.validationMessage))).then((r) => {
				e.enableButtons(), e.enableInput(), r ? e.showValidationMessage(r) : n === "deny" ? kn(e, t) : Mn(e, t);
			});
		}, kn = (e, t) => {
			let n = Z.innerParams.get(e);
			n.showLoaderOnDeny && dn(F()), n.preDeny ? (e.isAwaitingPromise = !0, Promise.resolve().then(() => b(n.preDeny(t, n.validationMessage))).then((n) => {
				n === !1 ? (e.hideLoading(), on(e)) : e.close({
					isDenied: !0,
					value: n === void 0 ? t : n
				});
			}).catch((t) => jn(e, t))) : e.close({
				isDenied: !0,
				value: t
			});
		}, An = (e, t) => {
			...
			let n = Z.innerParams.get(e);
			n.showLoaderOnConfirm && dn(), n.preConfirm ? (e.resetValidationMessage(), e.isAwaitingPromise = !0, Promise.resolve().then(() => b(n.preConfirm(t, n.validationMessage))).then((n) => {
				Y(M()) || n === !1 ? (e.hideLoading(), on(e)) : An(e, n === void 0 ? t : n);
			}).catch((t) => jn(e, t))) : An(e, t);
		};
		function Nn() {
			let e = Z.innerParams.get(this);
			if (!e) return;
			let t = Z.domCache.get(this);
			J(t.loader), V() ? e.icon && q(E()) : Pn(t), K([t.popup, t.actions], l.loading), t.popup.removeAttribute("aria-busy"), t.popup.removeAttribute("data-loading"), t.confirmButton.disabled = !1, t.denyButton.disabled = !1, t.cancelButton.disabled = !1;
		...
		let Pn = (e) => {
			...
			n.length ? q(n[0], "inline-block") : pe() && J(e.actions);
		};
		function Fn() {
		...
		function zn() {
			Z.focusedElement.set(this, document.activeElement), In(this, [
				"confirmButton",
				"denyButton",
				"cancelButton"
			], !0);
		...
		function Hn(e) {
			...
			H(t.validationMessage, e), t.validationMessage.className = l["validation-message"], n.customClass && n.customClass.validationMessage && G(t.validationMessage, n.customClass.validationMessage), q(t.validationMessage);
			let r = this.getInput();
			r && (r.setAttribute("aria-invalid", "true"), r.setAttribute("aria-describedby", l["validation-message"]), oe(r), G(r, l.inputerror));
		}
		function Un() {
			let e = Z.domCache.get(this);
			e.validationMessage && J(e.validationMessage);
			let t = this.getInput();
			t && (t.removeAttribute("aria-invalid"), t.removeAttribute("aria-describedby"), K(t, l.inputerror));
		}
		let Wn = {
			title: "",
			titleText: "",
			...
			scrollbarPadding: !0,
			topLayer: !1
		}, Gn = /* @__PURE__ */ "allowEscapeKey.allowOutsideClick.background.buttonsStyling.cancelButtonAriaLabel.cancelButtonColor.cancelButtonText.closeButtonAriaLabel.closeButtonHtml.color.confirmButtonAriaLabel.confirmButtonColor.confirmButtonText.currentProgressStep.customClass.denyButtonAriaLabel.denyButtonColor.denyButtonText.didClose.didDestroy.draggable.footer.hideClass.html.icon.iconColor.iconHtml.imageAlt.imageHeight.imageUrl.imageWidth.preConfirm.preDeny.progressSteps.returnFocus.reverseButtons.showCancelButton.showCloseButton.showConfirmButton.showDenyButton.text.title.titleText.theme.willClose".split("."), Kn = { allowEnterKey: void 0 }, qn = [
			"allowOutsideClick",
			"allowEnterKey",
			"backdrop",
			"draggable",
		...
		], Jn = (e) => Object.prototype.hasOwnProperty.call(Wn, e), Yn = (e) => Gn.indexOf(e) !== -1, Xn = (e) => Kn[e], Zn = (e) => {
			Jn(e) || p(`Unknown parameter "${e}"`);
		}, Qn = (e) => {
			qn.includes(e) && p(`The parameter "${e}" is incompatible with toasts`);
		}, $n = (e) => {
			let t = Xn(e);
			t && _(e, t);
		}, er = (e) => {
			...
			for (let t in e) Zn(t), e.toast && Qn(t), $n(t);
		};
		function tr(e) {
			let t = S(), n = T(), r = Z.innerParams.get(this);
			if (!n || U(n, r.hideClass.popup)) {
			...
			let i = nr(e), a = Object.assign({}, r, i);
			er(a), t && (t.dataset.swal2Theme = a.theme), Ct(this, a), Z.innerParams.set(this, a), Object.defineProperties(this, { params: {
		...
		let nr = (e) => {
			...
			return Object.keys(e).forEach((n) => {
				Yn(n) ? t[n] = e[n] : p(`Invalid parameter to update: ${n}`);
			}), t;
		};
		function rr() {
			var e;
			...
			t.popup && a.swalCloseEventFinishedCallback && (a.swalCloseEventFinishedCallback(), delete a.swalCloseEventFinishedCallback), typeof n.didDestroy == "function" && n.didDestroy(), (e = a.eventEmitter) == null || e.emit("didDestroy"), ir(this);
		}
		let ir = (e) => {
			ar(e), delete e.params, delete a.keydownHandler, delete a.keydownTarget, delete a.currentInstance;
		}, ar = (e) => {
			e.isAwaitingPromise ? (or(Z, e), e.isAwaitingPromise = !0) : (or(zt, e), or(Z, e), delete e.isAwaitingPromise, delete e.disableButtons, delete e.enableButtons, delete e.getInput, delete e.disableInput, delete e.enableInput, delete e.hideLoading, delete e.disableLoading, delete e.showValidationMessage, delete e.resetValidationMessage, delete e.close, delete e.closePopup, delete e.closeModal, delete e.closeToast, delete e.rejectPromise, delete e.update, delete e._destroy);
		}, or = (e, t) => {
		...
		var sr = /* @__PURE__ */ Object.freeze({
			...
			showValidationMessage: Hn,
			update: tr
		});
		let cr = (e, t, n) => {
			e.toast ? lr(e, t, n) : (fr(t), pr(t), mr(e, t, n));
		}, lr = (e, t, n) => {
			t.popup.onclick = () => {
				e && (ur(e) || e.timer || e.input) || n(Ot.close);
			};
		}, ur = (e) => !!(e.showConfirmButton || e.showDenyButton || e.showCancelButton || e.showCloseButton), dr = !1, fr = (e) => {
			e.popup.onmousedown = () => {
				e.container.onmouseup = function(t) {
					e.container.onmouseup = () => {}, t.target === e.container && (dr = !0);
				};
			};
		}, pr = (e) => {
			e.container.onmousedown = (t) => {
				t.target === e.container && t.preventDefault(), e.popup.onmouseup = function(t) {
					e.popup.onmouseup = () => {}, (t.target === e.popup || t.target instanceof HTMLElement && e.popup.contains(t.target)) && (dr = !0);
				};
			};
		}, mr = (e, t, n) => {
			t.container.onclick = (r) => {
				...
				r.target === t.container && v(e.allowOutsideClick) && n(Ot.backdrop);
			};
		}, hr = (e) => typeof e == "object" && !!e && "jquery" in e, gr = (e) => e instanceof Element || hr(e), _r = (e) => {
			let t = {};
			return typeof e[0] == "object" && !gr(e[0]) ? Object.assign(t, e[0]) : [
				"title",
				"html",
				"icon"
			].forEach((n, r) => {
				let i = e[r];
				typeof i == "string" || gr(i) ? t[n] = i : i !== void 0 && m(`Unexpected type of ${n}! Expected "string" or "Element", got ${typeof i}`);
			}), t;
		};
		function vr(...e) {
			return new this(...e);
		...
		let br = () => a.timeout && a.timeout.getTimerLeft(), xr = () => {
			if (a.timeout) return ve(), a.timeout.stop();
		}, Sr = () => {
			if (a.timeout) {
			...
			let e = a.timeout;
			return e && (e.running ? xr() : Sr());
		}, wr = (e) => {
			if (a.timeout) {
				let t = a.timeout.increase(e);
				return _e(t, !0), t;
			}
		}, Tr = () => !!(a.timeout && a.timeout.isRunning()), Er = !1, Dr = {};
		function Or(e = "data-swal-template") {
			Dr[e] = this, Er ||= (document.body.addEventListener("click", kr), !0);
		}
		let kr = (e) => {
			for (let t = e.target; t && t !== document; t = t.parentNode) for (let e in Dr) {
				let n = t.getAttribute && t.getAttribute(e);
				if (n) {
					Dr[e].fire({ template: n });
					return;
				}
			}
		};
		...
		let Nr = [
			...
			if (!t) return {};
			let n = t.content;
			return Hr(n), Object.assign(Fr(n), Ir(n), Lr(n), Rr(n), zr(n), Br(n), Vr(n, Nr));
		}, Fr = (e) => {
			let t = {};
			return Array.from(e.querySelectorAll("swal-param")).forEach((e) => {
				Ur(e, ["name", "value"]);
				let n = e.getAttribute("name"), r = e.getAttribute("value");
				!n || !r || (n in Wn && typeof Wn[n] == "boolean" ? t[n] = r !== "false" : n in Wn && typeof Wn[n] == "object" ? t[n] = JSON.parse(r) : t[n] = r);
			}), t;
		}, Ir = (e) => {
			let t = {};
			...
			return n && (Ur(n, ["type", "color"]), n.hasAttribute("type") && (t.icon = n.getAttribute("type")), n.hasAttribute("color") && (t.iconColor = n.getAttribute("color")), t.iconHtml = n.innerHTML), t;
		}, Br = (e) => {
			...
			for (let r in t) {
				...
				a && (Ur(a, []), n[i.replace(/^swal-/, "")] = a.innerHTML.trim());
			}
			return n;
		}, Hr = (e) => {
			let t = Nr.concat([
			...
			if (Jr(r, i, e), setTimeout(() => {
				Kr(r, i);
			}, 10), B() && (qr(r, e.scrollbarPadding === void 0 ? !1 : e.scrollbarPadding, o), Bt()), Ut && e.backdrop === !1 && i.scrollHeight > r.clientHeight && (r.style.pointerEvents = "auto"), !V() && !a.previousActiveElement && (a.previousActiveElement = document.activeElement), typeof e.didOpen == "function") {
				let t = e.didOpen;
				setTimeout(() => t(i));
			}
			...
			let n = S();
			n && (t.removeEventListener("animationend", Gr), t.removeEventListener("transitionend", Gr), n.style.overflowY = "auto", K(n, l["no-transition"]));
		}, Kr = (e, t) => {
			ge(t) ? (e.style.overflowY = "hidden", t.addEventListener("animationend", Gr), t.addEventListener("transitionend", Gr)) : e.style.overflowY = "auto";
		}, qr = (e, t, n) => {
			Wt(), t && n !== "hidden" && Qt(n), setTimeout(() => {
				e.scrollTop = 0;
			});
		}, Jr = (e, t, n) => {
			var r;
			(r = n.showClass) != null && r.backdrop && G(e, n.showClass.backdrop), n.animation ? (t.style.setProperty("opacity", "0", "important"), q(t, "grid"), setTimeout(() => {
				var e;
				(e = n.showClass) != null && e.popup && G(t, n.showClass.popup), t.style.removeProperty("opacity");
			}, 10)) : q(t, "grid"), G([document.documentElement, document.body], l.shown), n.heightAuto && n.backdrop && !n.toast && G([document.documentElement, document.body], l["height-auto"]);
		};
		var Yr = {
		...
		function Xr(e) {
			e.inputValidator || (e.input === "email" && (e.inputValidator = Yr.email), e.input === "url" && (e.inputValidator = Yr.url));
		}
		function Zr(e) {
		...
		function Qr(e) {
			Xr(e), e.showLoaderOnConfirm && !e.preConfirm && p("showLoaderOnConfirm is set to true, but preConfirm is not defined.\nshowLoaderOnConfirm should be used together with preConfirm, see usage example:\nhttps://sweetalert2.github.io/#ajax-request"), Zr(e), typeof e.title == "string" && (e.title = e.title.split("\n").join("<br />")), Ee(e);
		}
		let $r;
		var ei = /* @__PURE__ */ new WeakMap();
		class $ {
			constructor(...e) {
				r(this, ei, Promise.resolve({
					isConfirmed: !1,
					isDenied: !1,
					isDismissed: !0
				})), !(typeof window > "u") && ($r = this, this.params = Object.freeze(this.constructor.argsToParams(e)), this.isAwaitingPromise = !1, i(ei, this, this._main($r.params)));
			}
			_main(e, t = {}) {
				if (er(Object.assign({}, t, e)), a.currentInstance) {
					let e = zt.swalPromiseResolve.get(a.currentInstance), { isAwaitingPromise: t } = a.currentInstance;
					a.currentInstance._destroy(), t || e({ isDismissed: !0 }), B() && Vt();
				}
				a.currentInstance = $r;
				let n = ni(e, t);
				Qr(n), Object.freeze(n), a.timeout && (a.timeout.stop(), delete a.timeout), clearTimeout(a.restoreFocusTimeout);
				let r = ri($r);
				return Ct($r, n), Z.innerParams.set($r, n), ti($r, r, n);
			}
			then(e) {
				return n(ei, this).then(e);
			}
			finally(e) {
				return n(ei, this).finally(e);
			}
		}
		let ti = (e, t, n) => new Promise((r, i) => {
			let o = (t) => {
				e.close({
					isDismissed: !0,
					dismiss: t,
			...
			zt.swalPromiseResolve.set(e, r), zt.swalPromiseReject.set(e, i), t.confirmButton.onclick = () => {
				wn(e);
				...
				o(Ot.close);
			}, cr(n, t, o), At(a, n, o), pn(e, n), Wr(n), ii(a, n, o), ai(t, n), setTimeout(() => {
				t.container.scrollTop = 0;
			});
		}), ni = (e, t) => {
			let n = Pr(e), r = Object.assign({}, Wn, t, n, e);
			return r.showClass = Object.assign({}, Wn.showClass, r.showClass), r.hideClass = Object.assign({}, Wn.hideClass, r.hideClass), r.animation === !1 && (r.showClass = { backdrop: "swal2-noanimation" }, r.hideClass = {}), r;
		}, ri = (e) => {
			let t = {
				popup: T(),
				container: S(),
			...
			let r = ee();
			J(r), t.timer && (e.timeout = new Mr(() => {
				n("timer"), delete e.timeout;
			}, t.timer), t.timerProgressBar && r && (q(r), W(r, t, "timerProgressBar"), setTimeout(() => {
			...
			for (let e of t) if (e instanceof HTMLElement && Y(e)) return e.focus(), !0;
			return !1;
		}, si = (e, t) => t.focusDeny && Y(e.denyButton) ? (e.denyButton.focus(), !0) : t.focusCancel && Y(e.cancelButton) ? (e.cancelButton.focus(), !0) : t.focusConfirm && Y(e.confirmButton) ? (e.confirmButton.focus(), !0) : !1;
		$.prototype.disableButtons = zn, $.prototype.enableButtons = Rn, $.prototype.getInput = Fn, $.prototype.disableInput = Vn, $.prototype.enableInput = Bn, $.prototype.hideLoading = Nn, $.prototype.disableLoading = Nn, $.prototype.showValidationMessage = Hn, $.prototype.resetValidationMessage = Un, $.prototype.close = nn, $.prototype.closePopup = nn, $.prototype.closeModal = nn, $.prototype.closeToast = nn, $.prototype.rejectPromise = an, $.prototype.update = tr, $.prototype._destroy = rr, Object.assign($, jr), Object.keys(sr).forEach((e) => {
			$[e] = function(...t) {
				if ($r && $r[e]) return $r[e](...t);
			};
		}), $.DismissReason = Ot, $.version = "11.26.23";
		let ci = $;
		return ci.default = ci, ci;
	...
	createButtons() {
		let e = {
			...
			"export-json": {
				...
				action: () => {
					console.log(this.court.jsonExporter.export()), Te.default.fire({
						title: "Tática Pronta!",
	...
	createPlayer(e, t) {
		let n = new X.default.Group({
			x: e,
			y: t,
			draggable: !0,
			name: "player",
			id: `player-${Date.now()}`
		}), r = new X.default.Circle({
			radius: 15,
			fill: this.teamColor,
			stroke: "#2c3e50",
			strokeWidth: 2,
		...
		i.offsetX(i.width() / 2), i.offsetY(i.height() / 2), n.add(r), n.add(i), n.on("dragmove", () => this.constrainToPitch(n, 15)), n.on("dragend", () => this.court.stateManager.saveState()), n.on("dblclick dbltap", async () => {
			let e = i.text(), { value: t } = await Te.default.fire({
			...
			t && t.trim() !== "" && (i.text(t.substring(0, 3)), i.offsetX(i.width() / 2), i.offsetY(i.height() / 2), this.court.interactiveLayer.batchDraw(), this.court.stateManager.saveState());
		}), this.court.interactiveLayer.add(n), this.playerCount++, this.court.interactiveLayer.draw(), this.court.stateManager.saveState();
	}
	...
	onMouseDown(e) {
		this.isDrawing = !0;
		let t = this.court.stage.getPointerPosition();
		this.currentArrow = new X.default.Arrow({
			points: [
				t.x,
				t.y,
				t.x,
	...
	onMouseDown(e) {
		...
		this.shapeType === "rect" ? this.currentShape = new X.default.Rect({
			...n,
			width: 0,
			height: 0
		}) : this.currentShape = new X.default.Ellipse({
			...n,
			radiusX: 0,
			radiusY: 0
	...
	constructor(e, t = {}) {
		this.containerId = e, this.options = {
			...
			...t
		}, this.stateManager = new we(this), this.jsonExporter = new Me(this), this.imageExporter = new Ne(this), this.initKonva(), this.initTools(), this.options.toolbar !== !1 && (this.toolbar = new Ee(this, this.options.toolbar)), this.drawPitch();
	}
	initTools() {
		this.tools = {
			select: new Oe(this),
			playerA: new ke(this, "#3498db", "#ffffff"),
			playerB: new ke(this, "#e74c3c", "#ffffff"),
			arrow: new Ae(this, "#e74c3c"),
			rect: new je(this, "rect", "#f39c12"),
			ellipse: new je(this, "ellipse", "#f39c12")
		}, this.setTool(this.tools.select);
	}
	initKonva() {
		this.stage = new X.default.Stage({
			container: this.containerId,
			width: this.options.width,
			height: this.options.height
		}), this.bgLayer = new X.default.Layer(), this.interactiveLayer = new X.default.Layer(), this.transformer = new X.default.Transformer({
			nodes: [],
	...
	drawPitch() {
		let { width: e, height: t, backgroundColor: n, lineColor: r } = this.options, i = new X.default.Rect({
			x: 0,
			y: 0,
			width: e,
			height: t,
		...
		this.bgLayer.add(c);
		let l = (e - 40) * .18, u = (t - 40) * .55, d = new X.default.Rect({
			x: 20,
			y: t / 2 - u / 2,
			width: l,
			height: u,
		...
		this.bgLayer.add(f);
		let p = (e - 40) * .06, m = (t - 40) * .25, h = new X.default.Rect({
			x: 20,
			y: t / 2 - m / 2,
			width: p,
			height: m,
		...
		this.bgLayer.add(g), this.bgLayer.draw();
	}
}, Fe = /* @__PURE__ */ m(((e) => {
...
})))(), Re = ({ width: r = 800, height: i = 500, backgroundColor: a = "#2ecc71", lineColor: o = "#ffffff", onCanvasReady: s = null }) => {
	let c = t(null), [l, u] = n(null);
	return e(() => {
		if (!c.current) return;
		let e = `court-react-${Math.random().toString(36).substr(2, 9)}`;
		c.current.id = e;
		let t = new Pe(e, {
			width: r,
			height: i,
		...
		return t.jsonExporter = new Me(t), t.imageExporter = new Ne(t), u(t), s && s(t), () => {
			t.stage.destroy();
		};
	}, []), /* @__PURE__ */ (0, Le.jsx)("div", {
		ref: c,
	...
	emits: ["ready"],
	setup(e, { emit: t }) {
		let n = e, c = t, l = s(`court-vue-${Math.random().toString(36).substr(2, 9)}`), u = null;
		return a(() => {
			u = new Pe(l.value, {
				width: n.width,
				height: n.height,
			...
			}), u.jsonExporter = new Me(u), u.imageExporter = new Ne(u), c("ready", u);
		}), i(() => {
			u && u.stage && u.stage.destroy();
		}), (e, t) => (o(), r("div", {
			id: l.value,
...
//#endregion
export { Ae as ArrowTool, Pe as CourtCanvas, Re as CourtCanvasReact, Z as CourtCanvasVue, Ne as ImageExporter, Me as JsonExporter, ke as PlayerTool, Oe as SelectTool, je as ShapeTool };
